...
Действия инструмента вне мазка регистрируются вместе с движком через `defineBrushEngine({ ..., commands: { parse, run } })`. `parse` проверяет неизвестный payload, `run` получает типизированную команду, проверенные настройки, renderer и временно закреплённые ресурсы. Передача `{ type: 'brush-command', requestId, brush, command }` использует ту же очередь в обоих режимах; ответ содержит тот же `requestId` и `Result<void, string>`. Команды во время мазка отклоняются, не завершая его. Такие действия меняют состояние инструмента, но не пиксели документа, историю или статус сохранения. Например, ABR Mixer регистрирует `load`, `clean` и `{ type: 'load-canvas', point: { x, y } }` в координатах документа. Добавление команд нового движка не требует веток в worker.
...
Для догоняния неподвижного пера процессор может реализовать `idle(elapsedMs)`, возвращающий обработанные точки. Движок передаёт их renderer без повторной обработки и возвращает из своего `idle` значение `true`, чтобы показать результат и продолжить часы, или `false`, чтобы остановить их до следующего ввода. Runtime вызывает этот необязательный метод в общей очереди, не более одной ожидающей операции. Завершение, отмена, потеря устройства и disposal останавливают часы. Процессор None не предоставляет idle и сохраняет сырой ввод. При этом движок Airbrush может использовать свои часы для накопления краски, независимо от сглаживания пути.

Для декодированного ABR-наконечника есть `texturedBrush`, который растеризует покрытие в тайлы Studio. Экспериментальный инструмент ABR Brush открывает общий ABR-viewer для импорта и выбора наконечника; полный пресет передаётся движку `abrBrush`, включая поддерживаемую динамику и Tool Options. Сейчас `PaintRenderer` описывает существующий растровый backend Studio; контракта произвольных векторных документов здесь нет.
...
`@app-game/abr-paint/preset` создаёт снимок полного пресета и его ресурсов для `abrBrush`. Общая библиотека `packages/abr-brush` используется также viewer: настройки, динамика, сглаживание, процедурные наконечники и coverage-эффекты. Paint применяет размер до 5000 px, spacing до 1000%, roundness/flips, scatter, transfer, color dynamics, texture и dual tip. Flow/Opacity, режим наложения и pressure overrides берутся из сохранённых tool options. None обходит сглаживание пресета. Подробные ограничения и статус сравнения с Photoshop: [ABR painting engine](../../../../packages/abr-paint/README.md). Отдельный наконечник ограничен 8192 px / 32 MiB; весь пресет — 48 MiB CPU / 64 MiB GPU с mipmaps.
...
HTMLCanvasElement работает в главном потоке. OffscreenCanvas должен принадлежать среде, где работает renderer: передать его в worker можно один раз. Signals управляют уже доступными объектами; они не отменяют браузерные правила владения и передачи canvas. Переключатель main/worker делает checkpoint с `includeTools: true`, заменяет DOM-canvas и запускает рецепт в другой среде без перезагрузки страницы. `checkpointed.tools` содержит версионированное состояние инструментов растрового renderer; оно передаётся в `init.tools` до включения ввода. Mixer сохраняет резервуар, подхваченную краску, образец Auto Load и остаток загрузки. Три текстуры копируются без потерь rgba16float, всего 1.5 MiB. Ошибка чтения не разрешает уничтожить старый renderer. Обычное автосохранение и `.paint` не содержат этого временного состояния; восстановление после внезапной потери GPU его не гарантирует.
...
- `packages/abr-paint/src/resources.test.ts`: бюджет, LRU и атомарность ошибок кеша; `brushResources.test.ts`: освобождение ресурсов при сбое движка.
...
`studio/symmetry.ts` хранит и валидирует оси симметрии. `symmetryRenderer.ts` оборачивает заимствованный renderer на один мазок и преобразует отпечатки после BrushEngine. Копии используют общую транзакцию, opacity и Undo; для ABR меняются координаты и ориентация маски. Фоновые текстуры остаются в координатах документа. Дополнительных GPU-ресурсов нет.

Настройки осей передаются командой `symmetry` и входят в checkpoint/файл документа. `createPaintSession` предоставляет реактивные `symmetry`, `canUpdateSymmetry` и команду `updateSymmetry`, возвращающую false при приостановленных командах. Старые файлы открываются с Off. Сейчас runtime разрешает симметрию обычной круглой кисти и ABR Brush/Pencil/Eraser; сторонние движки и retouch не подключены. Расширяемая декларация этой возможности у BrushEngine остаётся отдельной задачей.
