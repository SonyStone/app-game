...
export async function verifyPatternRetouch(report: (message: string) => void) {
  let cases = 0;
  for (const type of ['SmTl', 'BlTl'] as const) for (const eachTip of [true, false]) {
    ...
    for (const [key, expected] of separate) {
      ...
      if (!actual || expected.some((byte, i) => byte !== actual[i]))
        throw new Error(`Pattern ${type}, eachTip=${eachTip}, tile ${key} changed with direct drawing/batching/eviction.`);
    }
    cases++;
...
async function render(type: 'SmTl' | 'BlTl', eachTip: boolean, optimized: boolean) {
  ...
  const values = brushToFormValues({ id: 'pattern-retouch', name: 'Pattern retouch', type: 'computed',
    settings: {}, diameter: 48, spacing: 25, hardness: 100 });
  Object.assign(values.tool, { type, strength: 90, fingerPainting: false });
  values.useTexture = true;
  Object.assign(values.texture, { eachTip, scale: 60, mode: 'Mltp', depth: 65, depthControl: 0, depthJitter: 0,
  ...
  resources.put({ id: 'pattern', width: 8, height: 7, format: 'r8unorm',
  ...
  const renderer = await createPaintRenderer(new OffscreenCanvas(512, 256), error => errors.push(error), {
    cacheTiles: optimized ? 1 : 16, directSmudge: optimized, batchSmudgePasses: optimized,
    batchSmudgeDabs: optimized, sharedScratch: optimized
  });
  try {
    const stroke = createResourceSession(resources, resources => abrBrush.engine({ resources, renderer,
      layer: document.active, layers: document.layers, processor: createRawProcessor(),
