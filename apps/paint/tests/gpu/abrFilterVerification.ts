    ...
    preset: {
      ...
      ...{
        toolOptions: {
          ...
          filterAllLayers: options.allLayers ?? false,
          detailBoost: options.protect ?? false,
          mode: { domain: 'BlnM', value: options.mode ?? 'Nrml' }
        }
      },
      tip: { kind: 'computed', diameter: pixels(options.size ?? 32), spacing: percent(25), hardness: percent(100) }
    },
    resources: [],
