  ...
  const run = async (
    ...
    const preset = prepareAbrBrush({
      ...
      preset: {
        ...
        sourceId: 'fixture',
        ...{
          toolOptions: { kind: 'SmTl', strength: strength, fingerPainting: finger, smudgeAllLayers: allLayers },
          ...(dual
            ? {
                dualBrush: {
                  kind: 'dualBrush',
                  enabled: true,
                  blendMode: { domain: 'BlnM', value: 'Mltp' },
                  tip: {
                    kind: 'computed',
                    diameter: pixels(32),
                    spacing: percent(12.5),
    ...
    for (const scenario of ['plain', 'allLayers', 'finger', 'dual'] as const) {
      ...
      for (const [key, expected] of reference.tiles) {
        ...
        if (!actual || expected.some((value, i) => Math.abs(value - actual[i]!) > 1)) {
          ...
          throw new Error(
            `Fused Smudge changed pixels: ${mixing}/${scenario}/${key}: ${changed} channels, maximum error ${maximum}/255.`
          );
        }
      }
    }
