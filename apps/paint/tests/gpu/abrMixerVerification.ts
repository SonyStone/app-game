...
import { initAbr, percent, pixels as px } from '@app-game/abr-parser';
...
async function renderMixerStroke(
  options: {
    wet?: number;
    mix?: number;
    load?: number;
  ...
  const preset = prepareAbrBrush({
    ...
    preset: {
      ...
      tip: { kind: 'computed', diameter: px(32), spacing: percent(12.5), hardness: percent(100) }
    },
    resources: [],
