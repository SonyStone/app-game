...
export async function verifyAbrBrush(
  report: (message: string) => void,
  paintbrushMaskOnly = false,
  onlyAccumulation = paintbrushMaskOnly,
  tipKind: 'computedBrush' | 'sampledBrush' = 'computedBrush',
  secondaryTipKind: 'sampledBrush' | 'computedBrush' = 'sampledBrush',
  size = 80,
  options: { dualBrush?: boolean; progress?: boolean; zoom?: number; lod?: number; adaptiveQuality?: boolean } = {}
) {
  ...
  const values = brushToFormValues({
    ...
    preset: {
      ...
      sourceId: 'fixture',
      ...{},
      tip: { kind: 'sampled', diameter: pixels(80), spacing: percent(15) }
    },
    resources: [],
  ...
  const run = async (capacity: number, batch: number, withPreview: boolean, batchSampledMasks = true) => {
    ...
    const renderer = await createPaintRenderer(new OffscreenCanvas(800, 400), (error) => errors.push(error), {
      cacheTiles: capacity,
      batchSampledMasks,
      onPaintProgress: options.progress ? async () => {} : undefined
    });
    try {
      const brush = {
        ...defaultBrush(),
    ...
    for (const [key, pixels] of reference) {
      for (const result of [evicted, batched]) {
        ...
        if (!actual || actual.length !== pixels.length || pixels.some((value, i) => value !== actual[i])) {
          ...
          throw new Error(
            `ABR ${secondaryTipKind} ${mode} tile ${key} changed with ${result === evicted ? 'eviction/preview' : 'batching'}; byte ${i}: ${pixels[i]} -> ${actual?.[i]}.`
          );
        }
      }
