...
import type { BrushLibrary as AbrFile } from '@app-game/abr-brush/library';
import type { Composition } from '@app-game/abr-parser';
import type { AbrFileWithMeta, BrushWithPreview } from './abr';
type HierarchyItem = Composition['hierarchy'][number];
...
export function collectBrushes(node: TreeNode): BrushWithPreview[] {
  if (node.kind === 'brush')
    return [{ ...node.brush, name: node.name, preset: { ...node.brush.preset, name: node.name } }];
 ...
 */
export function buildHierarchyItems(group: GroupNode): HierarchyItem[] {
  const hasSubGroups = group.children.some((c) => c.kind === 'group');
  if (!hasSubGroups) return group.children.map(() => ({ kind: 'preset' as const }));

  const items: HierarchyItem[] = [];
  for (const child of group.children) {
    if (child.kind === 'brush') {
      items.push({ kind: 'preset' });
    } else {
      items.push({ kind: 'group', name: child.name, uuid: child.uuid });
...
function buildHierarchyFromGroup(group: GroupNode): HierarchyItem[] {
  const items: HierarchyItem[] = [];
  items.push({ kind: 'group', name: group.name, uuid: group.uuid });
  for (const child of group.children) {
    if (child.kind === 'brush') {
      items.push({ kind: 'preset' });
    } else {
...
export function groupToAbrFile(group: GroupNode): AbrFile {
  ...
  return { brushes, hierarchy, sources: group.sourceFile?.sources ?? [], errors: [] };
}
    ...
    .map((node): TreeNode | null => {
      if (node.kind === 'brush') {
        const matches =
          ...
          (node.brush.preset.tip?.kind ?? '').toLowerCase().includes(q) ||
