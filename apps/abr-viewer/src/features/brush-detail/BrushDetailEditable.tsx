...
export function BrushDetailEditable(props: {
  ...
  const initialValues = untrack(() => brushToFormValues(props.brush));
  ...
  const toggleFeature = (
    field: { [K in keyof BrushFormValues]: BrushFormValues[K] extends boolean ? K : never }[keyof BrushFormValues]
  ) => {
    setFormValues((draft) => {
