...
import type { BrushFormValues } from '../brush-detail/brush-form-schema';
...
export function createPreviewStroke(input: PreviewInput, tip: Pick<BrushTipImage, 'width' | 'height'>): PreviewStroke {
  ...
  const random = rng(0x6d2b79f5);
  const colorRandom = rng(0x152dc2e1);
  const points = smoothPoints(input.path?.length ? input.path : syntheticPath(), input);
  const diameter =
    Math.max(1, Math.min(v.diameter * input.dpr, input.height * 0.58, input.width * 0.16)) * (input.tipScale ?? 1);
  const baseSize = input.path?.length
    ? v.diameter * input.dpr * (input.tipScale ?? 1)
    : diameter / (1 + (v.useScattering ? scatter.scatter / 100 : 0) * 1.5);
  const size = Math.min(baseSize, input.height * 2);
  const data: number[] = [];
  let nextDistance = 0,
    traveled = 0,
    step = 0;
  const initialDirection = Math.atan2(
    (points[1]?.y ?? points[0]!.y) - points[0]!.y,
    (points[1]?.x ?? points[0]!.x) - points[0]!.x
  );
  let strokeColor: [number, number, number] | undefined;
  function stamp(p: PreviewPoint, direction: number) {
    if (data.length / stampStride >= 16384) return size;
    ...
    const inputValue = (control: number, fade: number, minimum = 0) => {
      ...
      else if (control === 4 || control === 8) value = p.x;
    ...
    const copies = v.useScattering
      ...
      : 1;
    for (let copy = 0; copy < copies && data.length / stampStride < 16384; copy++) {
      ...
      data.push(
        p.x * input.width + Math.cos(direction) * along - Math.sin(direction) * across,
        p.y * input.height + Math.sin(direction) * along + Math.cos(direction) * across,
...
function smoothPoints(points: PreviewPoint[], input: PreviewInput) {
  if (!input.path?.length || !input.values.useSmoothing || !input.values.smoothing.amount) return points;
  const s = input.values.smoothing,
    amount = s.amount / 100;
  const output = [points[0]!];
  for (const point of points.slice(1)) {
    const previous = output.at(-1)!;
    const distance = Math.hypot((point.x - previous.x) * input.width, (point.y - previous.y) * input.height);
    const radius = amount * 30 * (s.adjustForZoom ? input.dpr : 1);
    const t = s.pulledString ? Math.max(0, (distance - radius) / Math.max(1, distance)) : 1 - amount * 0.9;
    output.push({ ...point, x: previous.x + (point.x - previous.x) * t, y: previous.y + (point.y - previous.y) * t });
    if (s.catchUp && distance < radius) output[output.length - 1] = interpolate(output.at(-1)!, point, 0.2);
  }
  if (s.catchUpAtEnd) output.push(points.at(-1)!);
  return output;
}
