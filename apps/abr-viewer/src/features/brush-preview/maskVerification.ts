...
export async function verifyMaskPreview() {
  ...
  const values = {
    ...brushToFormValues({ name: 'Mask QA', preset: { kind: 'brush', sourceId: 'preview', tip: { kind: 'sampled' } } }),
    diameter: 40,
    spacing: 10
  };
  ...
  try {
    ...
    const dynamic: PreviewInput = {
      ...input,
      color: '#e13b72',
      values: {
        ...values,
        useColorDynamics: true,
        colorDynamics: { ...values.colorDynamics, applyPerTip: true, hueJitter: 80 }
      }
    };
    const colored = await read(dynamic);
    const distinct = new Set(
      Array.from(
        { length: colored.length / 4 },
        (_, i) => `${colored[i * 4]},${colored[i * 4 + 1]},${colored[i * 4 + 2]}`
      )
    );
    ...
    secondary.path = [
      { ...secondary.path![0]!, x: 0.05 },
      { ...secondary.path![0]!, x: 0.95, time: 100 }
    ];
    ...
    return {
      paintedPixels: painted.length,
      colorVariants: distinct.size,
      resizeStable: true,
      modeSwitchStable: true,
      affineSecondaryCounts: [1, 2, 8],
      sampledSecondaryBatching: true,
      computedSecondary: true
    };
  } finally {
    gpu.dispose();
  }
