...
test('known path tangent controls first secondary scatter without delaying live contact', () => {
  ...
  const all = createAbrStrokeSampler({ ...input, initialTangent: { x: 1, y: 0 } }, source).add([point(0), point(50)]);
...
function settings(): PreviewInput & { size: number } {
  const values = brushToFormValues({
    ...
    preset: {
      ...
      sourceId: 'fixture',
      ...{},
      tip: { kind: 'computed', diameter: pixels(16), spacing: percent(25) }
    },
    resources: [],
    source: { format: 'photoshop-abr/v1' as const, bytes: new Uint8Array() }
  });
  Object.assign(values, {
    tipKind: 'sampledBrush',
    roundness: 100,
    angle: 14,
    spacing: 25,
    spacingEnabled: true,
