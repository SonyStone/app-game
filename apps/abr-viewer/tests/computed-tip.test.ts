...
import { prepareComputedTip, rasterizeComputedTip, computedSecondaryTip } from '@app-game/abr-brush/computedTip';
import { createMaskRoundingTable } from '@app-game/abr-brush/maskRounding';
import { computedRadialProfile } from '@app-game/abr-brush/computedTip';
...
test('secondary source caching follows size and hardness without mutating previous masks', () => {
  ...
  const first = computedSecondaryTip(32, settings), saved = first.data.slice();
...
test('preview resize regenerates computed secondary pixels and placement uses unit scale', () => {
  ...
  const withoutStoredAngle = createPreviewStroke({ ...secondary, values: { ...secondary.values, angle: 0, roundness: 100 } }, first.dualTip!);
...
test('computed row execution validates stride/capacity and preserves scalar output with one group', () => {
  ...
  for (const maximumThreads of [0, 1025, NaN, 1.5])
    expect(() => rasterizeComputedTip(tip, [2, 2], {...context, execution: {...execution, maximumThreads}})).toThrow(RangeError);
  for (const rowStride of [scalar.width - 1, 0x80000000, NaN, 64.5])
    expect(() => rasterizeComputedTip(tip, [2, 2], {...context, execution: {...execution, rowStride}})).toThrow(RangeError);
...
function fixture(): PreviewInput {
  const values = brushToFormValues({ id: 'computed', name: 'Computed', type: 'computed', settings: {}, diameter: 64, spacing: 25 });
  values.useDualBrush = true;
  Object.assign(values.dualBrush, { diameter: 32, hardness: 50, angle: 14, roundness: 50, count: 1 });
  return { values, width: 256, height: 128, dpr: 1, color: '#000000', background: '#ffffff', flow: 1, opacity: 1,
    path: [{ x: .5, y: .5, time: 0, pressure: 1, tiltX: 0, tiltY: 0, rotation: 0 }] };
}
