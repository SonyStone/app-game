...
function values() {
  const v = brushToFormValues({ id: 'depth', name: 'Depth', type: 'computed', settings: {} });
  v.useTexture = true;
...
test.each([
  ['Hght', 0, 64], ['Hght', 0.25, 80], ['Hght', 0.5, 96], ['Hght', 1, 128],
  ['linearHeight', 0, 64], ['linearHeight', 1, 128],
  ['Mltp', 0, 128], ['Mltp', 0.25, 113], ['Mltp', 0.5, 97], ['Mltp', 1, 65]
...
test('zero jitter or a zero range does not advance the depth random stream', () => {
  const v = values();
  const unexpected = () => { throw new Error('unexpected random draw'); };
...
test('Fade uses the fixed-point path, with the correct endpoints', () => {
  ...
  expect([0, 1, 2, 3, 4].map(step => textureDepth(v.texture, 1, step, () => 0.5) * 255))
    .toEqual([128, 107, 86, 64, 64]);
  v.texture.mode = 'Mltp';
  expect([0, 1, 2, 3].map(step => textureDepth(v.texture, 1, step, () => 0.5) * 255))
    .toEqual([64, 86, 107, 128]);
});

test('preview restores the depth stream and input batches do not change the stroke', () => {
  const v = values();
  v.texture.depthJitter = 90;
  const input = { values: v, size: 20, width: 200, height: 100, dpr: 1, color: '#000000', background: '#ffffff', flow: 1, opacity: 1, seed: 19 };
