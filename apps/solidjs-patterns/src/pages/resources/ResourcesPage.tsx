...
import { createMemo, createSignal, isPending, Loading, refresh } from 'solid-js';
...
function ResourceDemo(): JSX.Element {
  ...
  const post = createMemo(() => fetchPost(postId()));
  ...
  return (
    <Card class="flex flex-col gap-4">
      <div class="flex items-center gap-3">
        ...
        <Button size="sm" variant="ghost" onClick={() => refresh(post)}>
          ↺ refetch
        </Button>
      </div>

      <div class="text-xs text-neutral-500">
        State: <span class="font-mono text-violet-300">{isPending(post) ? 'pending' : 'ready'}</span>
      </div>

      <Loading fallback={<div class="animate-pulse text-xs text-neutral-500">Fetching post…</div>}>
