...
A common pattern is to accept an `as` prop, omit it from forwarded props, and render the final element through Dynamic.
...
function Button(props) {
  ...
  return <Dynamic component={props.as ?? 'button'} {...others} />;
}
```
...
function ValueProvider(props) {
  return <ValueContext value={newValue}>{props.children}</ValueContext>;
}
...
How does SolidJs works?

<SideBySideCode title="props-lowering.tsx">
...
type PropValue = AttributeValue | object | Function;
type PropsRecord = Record<string, PropValue>;

/**
 * Applies a props object to a node using reactive effects.
 */
