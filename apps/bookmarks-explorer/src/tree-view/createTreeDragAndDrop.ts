...
export function createTreeDragAndDrop<TNode, TItem, TTarget>(props: {
  ...
  /** Determines whether the operation copies or moves the source. */
  getDropEffect: (item: TItem, target: TTarget) => TreeDropEffect;
  /** Returns the target placement rendered by an optional drop indicator. */
  getPlacement: (target: TTarget) => TreeDropPlacement;
  /** Executes an accepted semantic drop. */
  onDrop: (item: TItem, target: TTarget) => Promise<void>;
    ...
    rowProps(node: TNode) {
      ...
      return {
        ...
        onDragStart(event: TreeDragEvent) {
          const item = props.createItem(node);
  ...
  function updateDropTarget(event: TreeDragEvent, node: TNode): void {
    ...
    event.preventDefault();
    event.stopPropagation();
    const effect = props.getDropEffect(dragItem, target);
