...
/** Owns camera and animation state for one mounted text viewer. */
export function createSceneRenderer() {
  ...
  let positions: { x: Float32Array; y: Float32Array } = { x: new Float32Array(), y: new Float32Array() };
  ...
  return {
    drawScene,
    camera: transform,
    /** Starts manual control at the displayed position, without jumping to the animation target. */
    stopAnimation() {
      Object.assign(transform, animTransform);
    },
    /** Makes direct manipulation immediate rather than interpolating behind the pointer. */
    syncCamera() {
      Object.assign(animTransform, transform);
      mustRenderNextFrame.value = true;
    },
    /** Supplies glyph centers used by the automatic camera tour. */
    setPositions(value: typeof positions) {
      positions = value;
    }
  };
  ...
  function drawScene(
    ...
    gl.uniformMatrix2fv(
      pageProgram.uniforms!.uRotation,
      false,
      rotationMatrix(animTransform.rotation, canvas.height / canvas.width)
    );
  ...
  function setPageUniforms(
    gl: WebGLRenderingContext,
    canvas: HTMLCanvasElement,
    program: ExtWebGLProgram,
    page: any,
    zoomx: number,
    zoomy: number
  ) {
    ...
    const matrix = rotationMatrix(animTransform.rotation, canvas.height / canvas.width);
    const corners = [
      [pageNdc.x0, pageNdc.y0],
      [pageNdc.x1, pageNdc.y0],
      [pageNdc.x0, pageNdc.y1],
      [pageNdc.x1, pageNdc.y1]
    ].map(([x, y]) => ({ x: matrix[0]! * x! + matrix[2]! * y!, y: matrix[1]! * x! + matrix[3]! * y! }));
    const bounds = {
    ...
    if (!boxesIntersect(bounds, viewportNdc)) {
      return false;
    }

    var aspect = canvas.height / canvas.width;

    gl.uniformMatrix2fv(program.uniforms!.uRotation, false, rotationMatrix(animTransform.rotation, aspect));
