...
export function fitFolderSlots(slots: readonly number[]) {
...
export function folderRows(order: readonly string[], left: (id: string) => number, width: number) {
  const rows = new Map<string, number>();
  order.forEach((id, rank) => {
    let row = restingRow(rank);
    for (const previous of order.slice(0, rank)) {
      const overlap = Math.abs(left(id) - left(previous)) < width;
      row = Math.max(row, rows.get(previous)! + (overlap ? 6.2 : 1.2));
    }
    rows.set(id, row);
  });
  return rows;
