...
import { createStrokePointKey, type Drawing, type Stroke, type StrokeId, type StrokePointKey } from '../document';
...
export function useDocumentSelection(params: DocumentSelectionParams) {
  const [selectedStrokeIds, setSelectedStrokeIds] = createSignal<ReadonlySet<StrokeId>>(new Set<StrokeId>());
  const [selectedPointKeys, setSelectedPointKeys] = createSignal<ReadonlySet<StrokePointKey>>(
  ...
  createTrackedEffect(() => {
    ...
    setSelectedPointKeys((current) => intersectPointKeys(current, getLivePointKeys(drawing?.strokes ?? [])));
  });
