...
import styles from './MaterialFillControls.module.css';
import {
...
export function MaterialFillControls(props: MaterialFillControlsProps) {
  return (
    <div class={styles.materialFillRow}>
      <label class={styles.selectControl}>
        Fill
        <select
          name="material-fill-style"
          value={props.fillStyle}
          onChange={(event) => props.onSetFillStyle(readMaterialFillStyle(event.currentTarget.value))}
        >
          <For each={materialFillStyles}>
            {(fillStyle) => <option value={fillStyle}>{materialFillStyleLabels[fillStyle]}</option>}
          </For>
        </select>
      </label>
      ...
      <label class={styles.selectControl}>
        ...
        <select
          ...
          onChange={(event) => props.onSetGradientType(readMaterialGradientType(event.currentTarget.value))}
        >
          <For each={materialGradientTypes}>
            {(gradientType) => <option value={gradientType}>{materialGradientTypeLabels[gradientType]}</option>}
          </For>
        </select>
      </label>
    </div>
