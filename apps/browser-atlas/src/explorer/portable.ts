...
/** Stable identifier written into every portable Browser Atlas document. */
export const EXPLORER_DOCUMENT_FORMAT = 'browser-atlas';

/** Current portable document schema version. */
export const EXPLORER_DOCUMENT_VERSION = 2;

/** Browser-independent tree node that can cross backends, browsers, and files. */
export type PortableExplorerNode =
  | PortableExplorerGroup
  | PortableExplorerLink
  | PortableExplorerNote
...
export type PortableExplorerLink = {
  ...
  defaultCollapsed: boolean;
  /** Explicit durable mark that keeps an opened copy in the tree after its browser tab closes. */
  keepOnClose?: true;
};

/** Portable annotation whose text and descendants survive browser and file boundaries. */
export type PortableExplorerNote = {
  /** Distinguishes annotations from navigable links and structural groups. */
  kind: 'note';
  /** User-authored annotation text. */
  text: string;
  /** Ordered portable descendants attached to this annotation. */
  children: PortableExplorerNode[];
  /** Whether viewers should initially collapse nested descendants. */
  defaultCollapsed: boolean;
};

/** Portable visual separator retaining its original Tabs Outliner style. */
export type PortableExplorerSeparator = {
  /** Distinguishes visual rules from other semantic node kinds. */
  kind: 'separator';
  /** Solid, double, or dashed separator appearance. */
  style: 0 | 1 | 2;
  /** Ordered portable descendants attached to this separator. */
  children: PortableExplorerNode[];
  /** Whether viewers should initially collapse nested descendants. */
  defaultCollapsed: boolean;
};
...
export function createPortableExplorerNode(node: ExplorerTreeNode): PortableExplorerNode | null {
  ...
  if (node.kind === 'link') {
    if (!node.url) {
      if (node.reference.kind === 'saved-note' || node.reference.kind === 'document-note') {
        return { kind: 'note', text: node.title, children, defaultCollapsed: node.defaultCollapsed };
      }
      if (node.reference.kind === 'saved-separator' || node.reference.kind === 'document-separator') {
        return {
          kind: 'separator',
          style: node.reference.style,
          children,
...
/** Converts only rows currently visible in a tree view while retaining their semantic parent-child structure. */
export function portableVisibleChildren(
  root: ExplorerTreeNode,
  visibleExpansion: ReadonlyMap<string, boolean>
): PortableExplorerNode[] {
  if (root.kind === 'message') {
    return [];
  }
  return root.children.flatMap((node) => createVisibleNode(node));

  function createVisibleNode(node: ExplorerTreeNode): readonly PortableExplorerNode[] {
    if (node.kind === 'message' || !visibleExpansion.has(node.id)) {
      return [];
    }
    const portableNode = createPortableExplorerNode(node);
    if (!portableNode) {
    ...
    return [{
      ...portableNode,
      defaultCollapsed:
        node.children.length > 0 ? visibleExpansion.get(node.id) === false : portableNode.defaultCollapsed,
      children: node.children.flatMap(createVisibleNode)
    }];
  }
}
...
export function parseExplorerDocument(serialized: string): ExplorerDocument {
  ...
  if (value.version !== EXPLORER_DOCUMENT_VERSION) {
    throw new Error(`Unsupported Browser Atlas document version: ${String(value.version)}.`);
  }
...
/** Creates a portable note from selected or pasted plain text. */
export function createPortableTextNote(text: string): PortableExplorerNote {
  const trimmed = text.trim();
  if (!trimmed) {
    throw new Error('A note cannot be created from empty text.');
  }
  return { kind: 'note', text: trimmed, children: [], defaultCollapsed: false };
}

/** Returns a stable user-facing label for drag feedback, downloads, and fallback folders. */
export function portableNodeTitle(node: PortableExplorerNode): string {
  switch (node.kind) {
    case 'group':
    case 'link':
      return node.title;
...
function parseNode(value: unknown, path: string, depth: number, state: ParseState): PortableExplorerNode {
  ...
  if (value.kind === 'link') {
    if (
      ...
      (value.keepOnClose !== undefined && value.keepOnClose !== true)
    ) {
    ...
    return {
      ...
      children,
      defaultCollapsed,
      ...(value.keepOnClose === true ? { keepOnClose: true } : {})
    };
  }

  if (value.kind === 'group' && typeof value.title === 'string' && isPortableGroupKind(value.groupKind)) {
  ...
  if (value.kind === 'note' && typeof value.text === 'string') {
    return { kind: 'note', text: value.text, children, defaultCollapsed };
  }

  if (value.kind === 'separator' && isSeparatorStyle(value.style)) {
    return { kind: 'separator', style: value.style, children, defaultCollapsed };
  }
...
function isSeparatorStyle(value: unknown): value is PortableExplorerSeparator['style'] {
  return value === 0 || value === 1 || value === 2;
}

function isPortableGroupKind(value: unknown): value is PortableExplorerGroup['groupKind'] {
  return value === 'window' || value === 'folder' || value === 'date' || value === 'group';
}
