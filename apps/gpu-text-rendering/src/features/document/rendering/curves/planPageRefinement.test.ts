...
const base = { page: 2, level: 0, x: 0, y: 0 };
const parent = { page: 2, level: 2, x: 0, y: 0 };

it('covers missing surroundings once before spending four jobs on finer regions', () => {
  const jobs = planPageRefinement(children, (key) => key === pageTileKey(base), true);
  expect(jobs).toEqual([{ tile: parent, source: pageTileKey(children[0]!) }]);
  expect(
    planPageRefinement(children, (key) => key === pageTileKey(parent) || key === pageTileKey(base), true).map(
      (job) => job.tile
    )
  ).toEqual(children);
});
...
it('does not delay zoom-in with intermediate levels, or replace already resident fine tiles', () => {
  expect(planPageRefinement(children, (key) => key === pageTileKey(base), false).map((job) => job.tile)).toEqual(
    children
  );
  expect(planPageRefinement(children, () => true, true).map((job) => job.tile)).toEqual(children);
});

it('prepares one missing page fallback before visible detail on first visit', () => {
  const jobs = planPageRefinement(children, () => false, false);
  expect(jobs.map((job) => job.tile)).toEqual([base, ...children]);
});
