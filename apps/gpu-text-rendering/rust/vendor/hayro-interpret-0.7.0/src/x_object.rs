...
pub(crate) fn draw_form_xobject<'a, 'b>(
    ...
    if x_object.is_transparency_group {
        let properties = x_object.dict.get::<Dict<'_>>(GROUP).unwrap_or_default();
        device.push_form_group(
            ...
            properties.get::<bool>(b"I").unwrap_or(false),
            properties.get::<bool>(b"K").unwrap_or(false),
        );
...
fn decode_raster(
    ...
    if obj.stream.dict().get::<u8>(SMASK_IN_DATA) == Some(2)
        && let Some(alpha) = &alpha
    {
        // Embedded associated alpha uses the image's Matte, defaulting to black.
        let matte = obj.stream.dict().get::<ColorComponents>(MATTE);
        let mut matte_rgb = [0_u8; 3];
        if let Some(matte) = matte
            && matte.len() == ctx.color_space.num_components() as usize
        {
            ctx.color_space.convert_f32(&matte, &mut matte_rgb, false);
        }
        unpremultiply(&mut image, &alpha.data, &matte_rgb);
    }
...
fn resolve_alpha(
    ...
    let alpha = if let Some(1 | 2) = dict.get::<u8>(SMASK_IN_DATA) {
