...
impl<'a> Stream<'a> {
    ...
    pub fn decoded_image(
        ...
        self.decode_filters(image_params, None)
    }

    /// Apply only the first `count` stream filters, preserving any remaining encoding.
    /// This allows callers to unwrap ASCII encodings without decoding JPEG pixels.
    /// Returns an error if `count` exceeds the number of filters. Results are not cached.
    pub fn decoded_prefix(&self, count: usize) -> Result<Cow<'a, [u8]>, DecodeFailure> {
        self.decode_filters(&ImageDecodeParams::default(), Some(count))
    ...
    fn decode_filters(
        ...
        let count = count.unwrap_or(filters_and_params.filters.len());
        if count > filters_and_params.filters.len() {
