...
(function (name, context, definition) {
  ...
  function QR(M) {
    ...
    // extract R
    for (i = 0; i < n; ++i)
      for (j = 0; j < n; ++j) R[i][j] = i < j ? qr[i][j] : i == j ? d[i] : 0;
    return [Q, R];
  }
  ...
  return function Algebra(p, q, r) {
    ...
    if (options instanceof Object) {
      ...
      p =
        p.p === undefined
          ? (p.metric && p.metric.filter((x) => x == 1).length) || 0
          : p.p || 0;
    } else {
      options = {};
      p = p | 0;
    ...
    if (options.dual || (p == 0 && q == 0 && r < 0)) {
      ...
      options.tot = r + 1;
      options.Cayley = [...Array(r + 1)].map((a, i) =>
        [...Array(r + 1)].map((y, j) =>
          i * j == 0 ? (i + j ? 'e0' + (i + j) : '1') : '0'
        )
      );
    }
    ...
    var tot = (options.tot =
      (options.tot ||
        (p || 0) + (q || 0) + (r || 0) ||
        (options.basis && options.basis.length)) | 0);
    ...
    var low = basis.length == 1 ? 1 : basis[1].match(/\d+/g)[0] * 1,
      grades =
        options.grades ||
        (options.dual && basis.map((x, i) => (i ? 1 : 0))) ||
        basis.map((x) => (tot > 9 ? (x.length - 1) / 2 : x.length - 1)),
      grade_start = grades
    ...
    if (
      ...
      var metric =
        (options.Cayley && options.Cayley.map((x, i) => x[i])) ||
        basis.map((x, xi) => simplify(x + x, p, q, r) | 0);
      metric[0] = 1;
      ...
      var mulTable =
        options.Cayley ||
        basis.map((x) =>
          basis.map((y) => (x == 1 ? y : y == 1 ? x : simplify(x + y, p, q, r)))
        );
      ...
      if (
        ...
        !options.Cayley &&
        r >= 0 &&
        options.basis.length != 2 ** tot
      ) {
        metric = metric.filter((x, i) => options.basis.indexOf(basis[i]) != -1);
        mulTable = mulTable
          .filter((x, i) => options.basis.indexOf(basis[i]) != -1)
          .map((x) =>
            x.filter((x, i) => options.basis.indexOf(basis[i]) != -1)
          );
        basis = options.basis;
      }
      ...
      basis.forEach((o, oi) => {
        ...
        gpo[o].forEach(([xi, yi]) => {
          ...
          cp[oi][xi] =
            (cp[oi][xi] == '0' ? '' : cp[oi][xi] + '+') +
            (grades[oi] == grades[yi] - grades[xi] ? gp[oi][xi] : '0');
          cps[oi][xi] =
            (cps[oi][xi] == '0' ? '' : cps[oi][xi] + '+') +
            (grades[oi] == Math.abs(grades[yi] - grades[xi])
              ? gp[oi][xi]
              : '0');
        });
      });
      ...
      var generator = class MultiVector extends (options.baseType ||
        Float32Array) {
        ...
        get UnDual() {
          if (r)
            return this.map((x, i, a) => a[drm[i]] * drms[a.length - i - 1]);
          var res = new this.constructor();
          res[res.length - 1] = 1;
          return this.Div(res);
        }
        get Length() {
          return options.over
            ? Math.sqrt(Math.abs(this.Mul(this.Conjugate).s.s))
            : Math.sqrt(Math.abs(this.Mul(this.Conjugate).s));
        }
        get VLength() {
          var res = 0;
      ...
      var basisg = grade_start
        .slice(0, grade_start.length - 1)
        .map((x, i) => basis.slice(x, grade_start[i + 1]));
      var counts = grade_start
        .map((x, i, a) => (i == a.length - 1 ? 0 : a[i + 1] - x))
        .slice(0, tot + 1);
      ...
      var metric = basisg.map((x, xi) =>
        x.map(
          (y, yi) =>
            simplify_bits(
              basis_bits[grade_start[xi] + yi],
              basis_bits[grade_start[xi] + yi]
            )[0]
        )
      );
      var drms = basisg.map((x, xi) =>
        x.map(
          (y, yi) =>
            simplify_bits(
              basis_bits[grade_start[xi] + yi],
              ~basis_bits[grade_start[xi] + yi] & ((1 << tot) - 1)
            )[0]
        )
      );
      ...
      var generator = class MultiVector extends Array {
        ...
        Add(b, r) {
          ...
          for (var i = 0, l = Math.max(this.length, b.length); i < l; i++)
            if (!this[i] ^ !b[i])
              ...
              for (
                var j = 0, m = Math.max(this[i].length, b[i].length);
                j < m;
                j++
              ) {
                if (
                  typeof this[i][j] == 'string' ||
                  typeof r[i][j] == 'string' ||
                  typeof b[i][j] == 'string'
                ) {
        ...
        Scale(s) {
          return this.map(
            (x) =>
              x && x.map((y) => (typeof y == 'string' ? y + '*' + s : y * s))
          );
        }
        ...
        Mul(b, r) {
          ...
          for (
            var i = 0, x, gsx;
            (gsx = grade_start[i]), (x = this[i]), i < this.length;
            i++
          )
            if (x)
              for (
                var j = 0, y, gsy;
                (gsy = grade_start[j]), (y = b[j]), j < b.length;
                j++
              )
                if (y)
                          ...
                          if (i == j && a == bb) {
                            ...
                            if (
                              ...
                              r[0][0] =
                                ...
                                '*' +
                                y[bb] +
                                (metric[i][a] != 1 ? '*' + metric[i][a] : '');
                              gotstring = true;
                            } else r[0][0] += x[a] * y[bb] * metric[i][a];
                          } else {
                            var rn = simplify_bits(
                                basis_bits[gsx + a],
                                basis_bits[gsy + bb]
                              ),
                              g = bc(rn[1]),
        ...
        get Length() {
          ...
          this.forEach(
            (g, gi) =>
              g && g.forEach((e, ei) => (res += (e || 0) ** 2 * metric[gi][ei]))
          );
        ...
        get VLength() {
          var res = 0;
          this.forEach(
            (g, gi) => g && g.forEach((e, ei) => (res += (e || 0) ** 2))
          );
          return Math.abs(res) ** 0.5;
        }
        get Reverse() {
    ...
    var res = class Element extends generator {
      ...
      Split(iter = 50) {
        var k = Math.floor((p + q + r) / 2),
          OB = this.map((x) => x),
          B = this.map((x) => x),
          m = 1;
        var Wi = [...Array(k)].map((r, i) => {
          m = m * (i + 1);
          var Wi = B.Scale(1 / m);
          B = B.Wedge(OB);
          return Wi;
        ...
        if (k < 3) {
          ...
          var matrix = [...Array(k)].map((r, i) =>
            [...Array(k)].map((c, j) =>
              j == k - 1 ? Wis[k - i - 1] : i - 1 == j ? 1 : 0
            )
          );
          var eigen = eigenValues(matrix, iter).sort((a, b) =>
            Math.abs(a) < Math.abs(b) ? -1 : 1
          );
        }
        Wi = [Element.Scalar(1), ...Wi, Element.Scalar(0)];
        var sum = Element.Scalar(0),
          res = eigen.slice(1).map((v) => {
            var r = Math.floor(k / 2),
              N = Element.Scalar(0),
              DN = Element.Scalar(0);
            for (var i = 0; i <= r; ++i) {
              N.Add(Wi[2 * i + 1].Scale(v ** (r - i)), N);
              DN.Add(Wi[2 * i].Scale(v ** (r - i)), DN);
            }
      ...
      Factorize(iter = 50) {
        ...
        R.push(
          R.reduce((tot, fact) => tot.Mul(fact.Reverse), Element.Scalar(1)).Mul(
            P
          )
        );
        //  }
      ...
      Exp(taylor = false) {
        if (!taylor && Math.abs(this[0]) < 1e-9 && !options.over) {
          return this.Grade(2)
            .Split()
            .reduce((total, simple) => {
              var square = simple.Mul(simple).s,
                len = Math.sqrt(Math.abs(square));
              if (len <= 1e-5) return total.Mul(Element.Scalar(1).Add(simple));
              if (square < 0)
                return total.Mul(
                  Element.Scalar(Math.cos(len)).Add(
                    simple.Scale(Math.sin(len) / len)
                  )
                );
              return total.Mul(
                Element.Scalar(Math.cosh(len)).Add(
                  simple.Scale(Math.sinh(len) / len)
                )
              );
            }, Element.Scalar(1));
        }
      ...
      Log(compat = false) {
        ...
        if (Math.abs(bwb[bwb.length - 1]) <= 1e-5 || Math.abs(this.s) <= 1e-5)
          return b.Scale(Math.atan2(s, this.s) / s);
        var p = bwb.Scale(-1 / (2 * s));
        return Element.Mul(
          Element.Mul(
            Element.Add(Math.atan2(s, this.s), p.Scale(1 / this.s)),
            b
          ),
          Element.Sub(s, p)
        ).Scale(1 / (s * s));
      ...
      static Add(a, b, res) {
        ...
        if ((a instanceof Array && !a.Add) ^ (b instanceof Array && !b.Add))
          return a instanceof Array
            ? a.map((x) => Element.Add(x, b))
            : b.map((x) => Element.Add(a, x));
      ...
      static Sub(a, b, res) {
        ...
        if (b && (a instanceof Array) ^ (b instanceof Array))
          return a instanceof Array
            ? a.map((x) => Element.Sub(x, b))
            : b.map((x) => Element.Sub(a, x));
        // If both are equal length arrays, add elements one-by-one
        if (
          b &&
          a instanceof Array &&
      ...
      static Mul(a, b, res) {
        ...
        if (a instanceof Array && b instanceof Array) {
          ...
          var r = a.map((x, i) =>
            b[0].map((y, j) => {
              var r = tot ? Element.Scalar(0) : 0;
              x.forEach(
                (xa, k) => (r = Element.Add(r, Element.Mul(xa, b[k][j])))
              );
              return r;
            })
          );
        ...
        if ((a instanceof Array) ^ (b instanceof Array))
          return a instanceof Array
            ? a.map((x) => Element.Mul(x, b))
            : b.map((x) => Element.Mul(a, x));
      ...
      static Wedge(a, b, res) {
        ...
        if (
          typeof a in { boolean: 1, number: 1 } &&
          typeof b in { boolean: 1, number: 1 }
        )
          return a ^ b;
        // Expressions
        while (a.call) a = a();
      ...
      static Conjugate(a) {
        ...
        if (a instanceof Array)
          return a[0].map((c, ci) =>
            a.map((r, ri) => Element.Conjugate(a[ri][ci]))
          );
      ...
      static lt(a, b) {
        ...
        while (b.call) b = b();
        return (
          (a instanceof Element ? a.Length : a) <
          (b instanceof Element ? b.Length : b)
        );
      }
      static gt(a, b) {
        while (a.call) a = a();
                  ...
                  x
                    ...
                    .map(
                      (x) =>
                        (x &&
                          (x[1].match(/-/) || ' ') +
                            String.fromCharCode(65 + 1 * x[1].match(/\d+/))) ||
                        ' 0'
                    )
                )
                .join('\n')
                ...
                .replace(
                  ...
                  (a, x) =>
                    ...
                    (metric[x] == 0 &&
                      grades[x] > 1 &&
                      (-1) ** grades[x] ==
                        (metric[basis.indexOf(basis[x].replace('0', ''))] ||
                          (-1) ** grades[x]))
                      ? '-'
                      : '') + basis[x]
                )
      ...
      static sum(B) {
        ...
        T2 = T2.map((x) =>
          x.map((y) =>
            y.match(/e/)
              ? y.replace(/(\d)/g, (x) => (x | 0) + max_T1)
              : y.replace('1', 'e' + (1 + max_T2 + max_T1))
          )
        );
        B2 = B2.map((y, i) =>
          i == 0
        ...
        var basis = [...B1, ...B2];
        var Cayley = T1.map((x, i) => [...x, ...T2[0].map((x) => '0')]).concat(
          T2.map((x, i) => [...T1[0].map((x) => '0'), ...x])
        );
        // Build the new algebra.
        var grades = [
          ...B1.map((x) => (x == '1' ? 0 : x.length - 1)),
          ...B2.map((x, i) => (i ? x.length - 1 : 0)),
        ];
        var a = Algebra({
          basis,
          Cayley,
      ...
      static graph(f, options) {
        ...
        var ww = options.width,
          ...
          project = (o) => {
            ...
            if (
              ...
              o = [
                ...
                .filter(
                  (x) =>
                    x &&
                    Math.abs(x[13]) <= (options.clip || 2) + 0.001 &&
                    Math.abs(x[12]) <= (options.clip || 2) + 0.001 &&
                    Math.abs(x[11]) <= (options.clip || 2) + 0.001
                );
              return o.map((o) =>
                tpcam
                  .Vee(options.camera.Mul(o).Mul(options.camera.Conjugate))
                  .Wedge(tpy)
              );
            }
            ...
            if (
              o instanceof Element &&
              o.length == 16 &&
              o.Grade(1).Length > 0.01
            ) {
              var m = Element.Add(
                  1,
                  Element.Mul(o.Normalized, Element.Coeff(3, 1))
                ).Normalized,
                e0 = 0;
              o = Element.sw(
                m,
                [
                  ...
                  Element.Trivector(x * o.Length, e0, z * o.Length, 1)
                )
              );
              return o.map((o) =>
                tpcam
            ...
            return tot == 4 && o instanceof Element && o.length == 16
              ? tpcam
                  .Vee(options.camera.Mul(o).Mul(options.camera.Conjugate))
                  .Wedge(tpy)
              : o.length == 2 ** tot
              ? Element.sw(options.camera, o)
              : o;
          };
        // gl escape.
        ...
        if (!(f instanceof Function) || f.length === 0) {
          ...
          var lx,
            ...
            to2d =
              tot == 5
                ? [0, 8, 11, 13, 19, 17, 22, 26]
                : tot == 3
                ? [0, 1, 2, 3, 4, 5, 6, 7]
                : [0, 7, 9, 10, 13, 12, 14, 15];
          ...
          function build(f, or) {
            ...
            var svg = new DOMParser().parseFromString(
              `<SVG viewBox="-2 -${2 * (hh / ww || 1)} 4 ${
                4 * (hh / ww || 1)
              }" style="width:${ww || 512}px; height:${
                hh || 512
                ...
                ? (() => {
                    if (tot == 4 && !options.conformal) {
                      const lines3d = (
                        ...
                        ...[...Array(n + 1)].map((x, i) => {
                          var f = from.map(
                              (x, i) => x * (i == 3 ? 1 : options.gridSize || 1)
                            ),
                            t = to.map(
                              (x, i) => x * (i == 3 ? 1 : options.gridSize || 1)
                            );
                          f[j] = t[j] =
                            ((i - n / 2) / (n / 2)) * (options.gridSize || 1);
                          ...
                          var ly2 =
                            (-options.scale * D2b[drm[3]]) / D2b[drm[1]];
                          var r = `<line x1="${lx}" y1="${ly}" x2="${lx2}" y2="${ly2}" stroke="black" stroke-width="${
                            i % 10 == 0 ? 0.005 : i % 5 == 0 ? 0.002 : 0.0005
                          }" />`;
                          if (l && i && i != n)
                            r += `<text text-anchor="middle" font-size="0.04" fill="black" x="${
                              l == 1 ? lx + ox : lx2 + ox
                            }" y="${oy + (l == 1 ? ly : ly2)}" >${(
                              (((from[j] < 0 ? -1 : 1) * (i - n / 2)) /
                                (n / 2)) *
                      ...
                      var front = Element.sw(
                          options.camera,
                          Element.Trivector(1, 0, 0, 0)
                        ).Dual.Dot(Element.Vector(0, 0, 0, 1)).s,
                        ff = front > 0 ? 1 : -1;
                      var left = Element.sw(
                          options.camera,
                          Element.Trivector(0, 0, 1, 0)
                        ).Dual.Dot(Element.Vector(0, 0, 0, 1)).s,
                        ll = left > 0 ? 1 : -1;
                      ...
                      [...Array(2 * n + 1)].map(
                        (x, xi) =>
                          `<line x1="${
                            dir
                              ? -10
                              : ((xi - n) * space - (tot < 4 ? 2 * cy : 0)) * s
                          }" y1="${
                            dir
                              ? ((xi - n) * space - (tot < 4 ? 2 * cx : 0)) * s
                              : -10
                          }" x2="${
                            dir
                              ? 10
                      ...
                      [
                        ...
                        .concat(
                          options.labels
                            ? [...Array(4 * n + 1)].map((x, xi) =>
                                xi - n * 2 == 0
                                  ? ``
                                  : `<text text-anchor="middle" font-size="0.05" x="${
                                      ((xi - n * 2) * 0.2 -
                                        (tot < 4 ? 2 * cy : 0)) *
                                      s
                                    }" y="0.06" >${((xi - n * 2) * 0.2).toFixed(
                                      1
                                    )}</text>`
                              )
                            : []
                        ...
                        .concat(
                          options.labels
                            ? [...Array(4 * n + 1)].map(
                                (x, xi) =>
                                  `<text text-anchor="end" font-size="0.05" y="${
                                    ((xi - n * 2) * 0.2 -
                                      (tot < 4 ? 2 * cx : 0)) *
                                      s -
                                    0.01
                                  }" x="-0.01" >${((xi - n * 2) * -0.2).toFixed(
                                    1
                                  )}</text>`
                              )
                            : []
                  ...
                  f.map((o, oidx) => {
                    ...
                    if (
                      ...
                      requestAnimationFrame(() => {
                        var r = build(
                          origf,
                          !res || document.body.contains(res)
                        ).innerHTML;
                    ...
                    var dash_for_r2 = (r2, render_r, target_width) => {
                      ...
                      var width =
                        half_circum /
                        Math.max(Math.round(half_circum / target_width), 1);
                      return `${width} ${width}`;
                    };
                    ...
                    if (o instanceof Array) {
                      ...
                      return o.length > 2
                        ...
                        : `<LINE style="pointer-events:none" x1=${
                            o[0].e1 * sc
                          } y1=${-o[0].e2 * sc} x2=${o[1].e1 * sc} y2=${
                            -o[1].e2 * sc
                          } stroke="${color || '#888'}"/>`;
                    }
                    ...
                    if (typeof o == 'string') {
                      var res2 =
                        o[0] == '_'
                          ? ''
                          : `<text x="${lx}" y="${ly}" font-family="Verdana" font-size="${
                              options.fontSize * 0.1 || 0.1
                    ...
                    if (!is_flat && b0 && !b1 && !b2) {
                      ...
                      ly = sc * -o.e2;
                      lr = 0;
                      return (res2 = `<CIRCLE onmousedown="this.parentElement.sel=${oidx}" cx="${lx}" cy="${ly}" r="${
                        pointRadius * 0.03
                      }" fill="${color || 'green'}"/>`);
                      ...
                      return `<LINE style="pointer-events:none" x1=${
                        lx - 10
                      } y1=${ly} x2=${lx + 10} y2=${ly} stroke="${
                        color || '#888'
                      ...
                      return `<CIRCLE onmousedown="this.parentElement.sel=${oidx}" cx="${lx}" cy="${ly}" r="${r}" fill="none" stroke="${
                        color || 'green'
                      }" stroke-dasharray="${dash_for_r2(
                        r2,
                        r,
                        lineWidth * 0.02
                      )}"/>`;
                      ...
                      var ei = ni,
                        ...
                        attitude = ei
                          .Wedge(eo)
                          .LDot(nix)
                          .Normalized.Mul(Element.Scalar(r)),
                        pos = o.Div(nix);
                      ...
                      ly = sc * -pos.e2;
                      if (sqr == 0)
                        return `<CIRCLE onmousedown="this.parentElement.sel=${oidx}" cx="${lx}" cy="${ly}" r="${
                          pointRadius * 0.03
                        }" stroke-width="${
                          lineWidth * 0.01
                        }" fill="none" stroke="${color || 'green'}"/>`;
                      ...
                      var res2 = `<CIRCLE onmousedown="this.parentElement.sel=${oidx}" cx="${lx}" cy="${ly}" r="${
                        pointRadius * 0.03
                      }" fill="${fill}" stroke-width="${
                        lineWidth * 0.01
                      }" stroke="${stroke}" stroke-dasharray="${dash_for_r2(
                        sqr,
                        pointRadius * 0.03,
                        lineWidth * 0.02
                      )}" />`;
                      ...
                      return (
                        res2 +
                        `<CIRCLE onmousedown="this.parentElement.sel=${oidx}" cx="${lx}" cy="${ly}" r="${
                          pointRadius * 0.03
                        }" fill="${fill}" stroke-width="${
                          lineWidth * 0.01
                        }" stroke="${stroke}" stroke-dasharray="${dash_for_r2(
                  ...
                  f
                    .map((o, oidx) => {
                      ...
                      if (o instanceof Array && o.length) {
                        ...
                        return o.length > 2
                          ...
                          : `<LINE style="pointer-events:none" x1=${
                              ...
                              (-options.scale * o[0][drm[3]]) / o[0][drm[1]]
                            } x2=${
                              (options.scale *
                                (drm[1] == 6 || drm[1] == 14 ? -1 : 1) *
                                o[1][drm[2]]) /
                              ...
                              (-options.scale * o[1][drm[3]]) / o[1][drm[1]]
                            } stroke="${color || '#888'}"/>`;
                      }
                      // svg
                      if (typeof o == 'string' && o[0] == '<') {
                      ...
                      if (o[to2d[2]] ** 2 + o[to2d[3]] ** 2 > 0.0001) {
                        ...
                        lx = 0.5;
                        ly =
                          options.scale * (drm[1] == 6 ? -1 : -1) * o[to2d[1]];
                        lr =
                          (-Math.atan2(o[to2d[2]], o[to2d[3]]) / Math.PI) * 180;
                        var res2 = `<LINE style="pointer-events:none" x1=-10 y1=${-ly} x2=10 y2=${-ly} stroke="${
                          color || '#888'
                      ...
                      if (o[to2d[4]] ** 2 + o[to2d[5]] ** 2 > 0.0001) {
                        ...
                        ly += 0.05;
                        lx += 0.1;
                        var res2 = `<LINE style="pointer-events:none" x1=${lx} y1=${-ly} x2=${
                          lx - o.e02
                        } y2=${-(ly + o.e01)} stroke="${color || '#888'}"/>`;
          ...
          res.onwheel = (e) => {
            ...
            options.scale = Math.min(
              5,
              Math.max(0.1, (options.scale || 1) - e.deltaY * 0.0001)
            );
            if (!anim) {
              var r = build(
                origf,
          ...
          res.onmousemove = (e) => {
            if (cammove && tot == 4 && !options.conformal) {
              ...
              var [dx, dy] = [
                (options.scale || 1) * (e.clientX - mousex) * 3,
                3 * (options.scale || 1) * (e.clientY - mousey),
              ];
              ...
              if (res.sel && f[res.sel].set) {
                ...
                if (options.camera)
                  options.camera.set(
                    Element.Bivector(0, 0, 0, 0, 0, options.p)
                      .Exp()
                      .Mul(
                        Element.Bivector(0, 0, 0, 0, options.h, 0).Exp()
                      ) /*.Mul(options.camera)*/
                  );
              }
              if (!anim) {
            ...
            var resx = res.getBoundingClientRect().width,
              ...
              x =
                ((e.clientX - res.getBoundingClientRect().left) /
                  (resx / 4 || 128) -
                  2) *
                (resx > resy ? resx / resy : 1),
              y =
                ((e.clientY - res.getBoundingClientRect().top) /
                  (resy / 4 || 128) -
                  2) *
                (resy > resx ? resy / resx : 1);
            x /= options.scale;
            y /= options.scale;
            if (options.conformal) {
              ...
              f[res.sel][drm[2]] =
                (drm[1] == 6 ? -x : x) - (tot < 4 ? 2 * options.camera.e01 : 0);
        ...
        function drawGrid() {
          const [X, Y] = [
            (x) => ((x - x_from) * w) / (x_to - x_from),
            (y) => ((y - y_from) * h) / (y_to - y_from),
          ];
          ...
          for (var i = x_from, j = y_from, ii = 0; ii <= 10; ++ii) {
            ...
            context.stroke();
            if (j.toFixed(1) != 0)
              context.fillText(j.toFixed(1), X((x_to - x_from) / 100), Y(j));
          }
        }
        ...
        if (f.length == 2)
          for (var px = 0; px < w; px++)
            for (var py = 0; py < h; py++) {
              var res = f(
                (px / w) * (x_to - x_from) + x_from,
                (py / h) * (y_to - y_from) + y_from
              );
              res = res.buffer
                ? [].slice.call(res)
                : res.slice
        ...
        // one parameter function.. go over x range, use result as y.
          ...
          for (var px = 0; px < w; px++) {
            ...
            res = Math.round(
              (res / (y_to - y_from) + -y_from / (y_to - y_from)) * h
            );
            if (res > 0 && res < h - 1)
      ...
      static graphGL2(f, options) {
        ...
        if (options.width && options.width.match && options.width.match(/px/i))
          canvas.width =
            ...
            (options.devicePixelRatio || devicePixelRatio || 1);
        if (
          options.height &&
          options.height.match &&
          options.height.match(/px/i)
        )
          canvas.height =
        ...
        var compile = (vs, fs) => {
          var s = [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].map((t, i) => {
            ...
            gl.compileShader(r);
            return (
              (gl.getShaderParameter(r, gl.COMPILE_STATUS) && r) ||
              console.error(gl.getShaderInfoLog(r))
            );
          });
        ...
        var draw = function (
          ...
          gl.uniformMatrix4fv(gl.getUniformLocation(p, 'p'), false, [
            5,
            0,
            0,
            0,
          ...
          gl.uniform3fv(
            gl.getUniformLocation(p, 'color'),
            new Float32Array(color)
          );
          gl.uniform3fv(
            gl.getUniformLocation(p, 'color2'),
            new Float32Array(color2)
          );
          if (color3)
            gl.uniform3fv(
              gl.getUniformLocation(p, 'color3'),
              new Float32Array(color3)
            );
          if (b)
            gl.uniform1fv(
        ...
        var programs = [],
          genprog = (grade) =>
            compile(
              `${gl2 ? '#version 300 es' : ''}
             ${gl2 ? 'in' : 'attribute'} vec4 position; ${
                gl2 ? 'out' : 'varying'
              } vec4 Pos; uniform mat4 mv; uniform mat4 p;
             ...
             float product_len (in float z, in float y, in float x, in float[${
               counts[grade]
             }] b) {
                ${this.nVector(options.up.length > tot ? 2 : 1, [])[
                  options.IPNS ? 'IPNS_GLSL' : 'OPNS_GLSL'
                ](this.nVector(grade, []), options.up)}
                return sqrt(abs(sum));
             }
             ...
             void main() {
               ...
               vec3 d2 = find_root( p , dir, ${
                 grade != tot - 1 ? options.thresh || 0.2 : '0.0075'
               } );
               ...
               if (dl2>0.0) {
                 ...
                 ${
                   gl2 ? 'col' : 'gl_FragColor'
                 } = vec4(max(0.2,abs(dot(n,normalize(L-d2))))*color3 + pow(abs(dot(n,normalize(normalize(L-d2)+dir))),100.0),1.0);
               } else discard;
             }`
            ),
          genprog2D = (grade) =>
            compile(
             ...
             float product_len (in float z, in float y, in float x, in float[${
               counts[grade]
                ...
                ${this.nVector(1, [])[options.IPNS ? 'IPNS_GLSL' : 'OPNS_GLSL'](
                  this.nVector(grade, []),
                  options.up
                )}
                return sqrt(abs(sum));
        ...
        canvas.update = (x) => {
          ...
          for (var i = 0, ll = x.length; i < ll; i++) {
            ...
            if (e instanceof Element) {
              var tt = options.spin
                ? (-performance.now() * options.spin) / 1000
                : -options.h || 0;
              tt += Math.PI / 2;
              ...
              while (!e[g] && g > 1) g--;
              if (!programs[tot - 1 - g])
                programs[tot - 1 - g] = options.up.find(
                  (x) => x.match && x.match('z')
                )
                  ? genprog(g)
                  : genprog2D(g);
        ...
        canvas.oncontextmenu = canvas.onmousedown = (e) => {
          ...
          var rc = canvas.getBoundingClientRect(),
            ...
            my =
              ((((e.y - rc.top) / (rc.bottom - rc.top)) * -4 + 2) *
                canvas.height) /
              canvas.width;
          canvas.onwheel = (e) => {
            e.preventDefault();
          ...
          canvas.onmousemove = (e) => {
            ...
            var mx = (e.movementX / (rc.right - rc.left)) * 2,
              my =
                ((e.movementY / (rc.bottom - rc.top)) * -2 * canvas.height) /
                canvas.width;
            if (sel == -2) {
        ...
        if (options && options.still) {
          ...
          canvas.im = i;
          return (
            requestAnimationFrame(canvas.update.bind(canvas, f, options)), i
          );
        } else
          return (
            requestAnimationFrame(canvas.update.bind(canvas, f, options)),
      ...
      static graphGL(f, options) {
          ...
          mtx = (x, iscam = true) => {
            ...
            if (tot == 5)
              return [
                ...
                0,
                xx * ct + z * -st,
                y * ct2 + (xx * st + z * ct) * -st2,
                y * st2 + xx * st + z * ct * ct2 + zoom,
                1,
              ];
            x = x.Normalized;
            var y = x.Mul(x.Dual),
        ...
        var programSphere = compile(
          `attribute vec4 position; varying vec4 Pos; varying vec3 N; uniform mat4 mv; uniform mat4 p;
        ...
        var programPoint = compile(
          `attribute vec4 position; varying vec4 Pos; uniform mat4 mv; uniform mat4 p;
                 void main() { gl_PointSize=${(
                   (options.pointRadius || 1) *
                   (options.devicePixelRatio || devicePixelRatio || 1) *
                   8.0
                 ).toFixed(2)}; Pos=mv*position; gl_Position = p*Pos; }`,
          `precision highp float; uniform vec3 color; uniform vec3 color2; varying vec4 Pos;
 ...
 //                  gl_FragColor = vec4(color2,(1.0-pow(abs(tc.x),2.0))*tc.y); 
        ...
        var fw = 33,
          ...
          ctx = Object.assign(font.getContext('2d'), {
            font: 'bold 48px lucida console, monospace',
          }),
          ftx = gl.createTexture();
        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, ftx);
        for (var i = 33; i < 127; i++)
          ctx.fillText(String.fromCharCode(i), (i - 33) * fw, 40);
        var specialChars = '∞≅¹²³₀₁₂₃₄₅₆₇₈₉⋀⋁∆⋅';
        specialChars
          .split('')
          .forEach((x, i) => ctx.fillText(x, (i - 33 + 127) * fw, 40));
        ...
        const line_to_tri = ([ax, ay, az, bx, by, bz]) => [
          ax,
          ay,
          az,
          ax,
        ...
        const line_to_tri2 = ([ax, ay, az, bx, by, bz]) => [
          bx,
          by,
          bz,
          bx,
        ...
        var interprete = (x) => {
          ...
          if (opnixzero && ipnixzero) {
            ...
            if (x.Grade(3).VLength) {
              ...
              normal = Element.LDot(
                Element.Mul(attitude, 1 / weight2),
                I3
              ).Normalized;
              ...
              btg = [...r.Mul(Element.Coeff(2, 1)).Mul(r.Conjugate)].slice(
                1,
                4
              );
            ...
            pos = [...Element.LDot(1 / ni.LDot(x).s, x).slice(1, 4)].map(
              (x) => -x
            );
            tp = 1;
          } else if (!x2zero) {
            ...
            pos = [...x.Mul(ni).Mul(x).slice(1, 4)].map(
              (x) => -x / (2.0 * nix2)
            );
        ...
        canvas.update = (x) => {
          ...
          lastpos = options.camera.Conjugate.Mul(
            ((a = new this()).set(lastpos, 11), a)
          )
            .Mul(options.camera)
            .slice(11, 14);
          // Grid.
          if (options.grid) {
            ...
            for (var j = 0; j < 3; ++j)
              for (var i = 0; i < options.gridLines[j].length; i += 6) {
                ltest.push(
                  ...line_to_tri(options.gridLines[j].slice(i, i + 6))
                );
                ltest2.push(
                  ...line_to_tri2(options.gridLines[j].slice(i, i + 6))
                );
                var w =
                  ((i / 12) | 0) % 10 == 0
                    ? 2
            ...
            draw(
              ...
              gl.TRIANGLES,
              ltest,
              [canvas.width / canvas.height, 0.003, 0.0],
              [0, 0, 0],
          ...
          for (var i = 0, ll = x.length; i < ll; i++) {
              ...
              if (d.tp == 4) {
                ...
                for (var j = 0; j < 65; j++) {
                  ne = d.pos.map(
                    (x, i) =>
                      x +
                      Math.cos((j / 32) * Math.PI) * d.weight2 * d.tg[i] +
                      Math.sin((j / 32) * Math.PI) * d.weight2 * d.btg[i]
                  );
              ...
              if (d.tp == 5) {
                if (!sphere) {
                  ...
                  for (var j = 0; j < W + 1; j++)
                    for (var k = 0; k < H; k++) {
                      pnts.push([
                        S((2 * pi * j) / W) * S((pi * k) / (H - 1)),
                        C((2 * pi * j) / W) * S((pi * k) / (H - 1)),
                        C((pi * k) / (H - 1)),
                      ]);
                      if (j && k) {
                ...
                gl.blendColor(1, 1, 1, 1 - (alpha || 0.1));
              ...
              if (i == ll - 1 || d.tp == 0) {
                ...
                if (e.call && e.length == 1 && !e.va2) {
                  ...
                  for (var ii = 0; ii < countx; ii++) {
                    temp.set(
                      Element.sw(e(ii / (countx - 1)), no).slice(1, 4),
                      ii * 3
                    );
                ...
                if (e instanceof Object && e.motor) {
                  if (!e.va || e.recalc) {
                    ...
                    for (
                      var i = 0;
                      i <
                      (e.zRange * e.xRange * e.yRange === 0
                        ? 2500
                        : Math.pow(e.zRange * e.xRange * e.yRange, 1 / 3) *
                          12000);
                      i++
                    ) {
                      var [x, y, z] = [
                        random() * (2 * e.xRange) - e.xRange,
                        random() * 2 * e.yRange - e.yRange,
                        random() * 2 * e.zRange - e.zRange,
                      ];
                ...
                if (typeof e == 'string') {
                  if (options.htmlText) {
                    ...
                    Object.assign(div.style, {
                      ...
                      pointerEvents: 'none',
                      left:
                        rc.left + (rc.right - rc.left) * (pos2[0] / 2 + 0.5),
                      top:
                        rc.top +
                        (rc.bottom - rc.top) * (-pos2[1] / 2 + 0.5) -
                        20,
                    });
                    if (div.last != e) {
                      div.innerHTML = e;
                      div.last = e;
                      if (self.renderMathInElement)
                        self.renderMathInElement(div);
                    }
                  } else {
                    gl.enable(gl.BLEND);
                    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
            ...
            if (e instanceof Element && e.Grade(1).Length > 0.001) {
              var m = Element.Add(
                  1,
                  Element.Mul(e.Normalized, Element.Coeff(3, 1))
                ).Normalized,
                e0 = 0;
              e = Element.sw(
                m,
                [
                  ...
                  Element.Trivector(x * e.Length, e0, z * e.Length, 1)
                )
              );
            }
            ...
            if (e instanceof Element && e.Grade(2).Length)
              e = [
                e
                  ...
                  .Wedge(e)
                  .Add(
                    e.Wedge(Element.Coeff(1, 1)).Mul(Element.Coeff(0, -500))
                  ),
                e
                  .LDot(e14)
                  .Wedge(e)
            ...
            if (e.call && e.length == 1) {
              var count = e.dx || 64;
              for (
                var ismot, xx, o = new Float32Array(3), ii = 0;
                ii < count;
                ii++
              ) {
                ...
                var m = e(ii / (count - 1));
                if (ii == 0)
                  ismot = m[0] || m[5] || m[6] || m[7] || m[8] || m[9] || m[10];
                xx = ismot
                  ? sw_mot_orig(m, o)
                  : m
                      .slice(11, 14)
            ...
            if (e.call && e.length == 2 && !e.va) {
              ...
              for (var pp = 0, ii = 0; ii < countx; ii++)
                for (var jj = 0; jj < county; jj++, pp += 3)
                  temp.set(
                    sw_mot_orig(e(ii / (countx - 1), jj / (county - 1)), o),
                    pp
                  );
              for (ii = 0; ii < countx - 1; ii++)
                for (var jj = 0; jj < county; jj++)
            ...
            // we could also be an object with cached vertex array of triangles ..
              ...
              if (!e.va) {
                ...
                e.va = createVA(
                  ...
                  undefined,
                  e.idx,
                  e.color ? new Float32Array(e.color) : undefined
                );
                e.va.tcount =
                  e.idx && e.idx.length ? e.idx.length : e.data.length * 3;
              }
              // render the vertex array.
                ...
                for (var ii = 0; ii < 4; ++ii)
                  for (var jj = 0; jj < 4; ++jj)
                    for (var kk = 0; kk < 4; ++kk)
                      M[ii * 4 + kk] += M1[ii * 4 + jj] * M2[jj * 4 + kk];
              }
              if (alpha) {
                gl.enable(gl.BLEND);
                gl.blendFunc(gl.CONSTANT_ALPHA, gl.ONE_MINUS_CONSTANT_ALPHA);
            ...
            // if we're a number (color), label or the last item, we output the collected items.
              ...
              if (typeof e == 'string') {
                if (options.htmlText) {
                  if (!canvas['_' + i]) {
                    console.log('creating div');
                    Object.defineProperty(canvas, '_' + i, {
                      value: document.body.appendChild(
                        document.createElement('div')
                      ),
                      enumerable: false,
                  ...
                  Object.assign(div.style, {
                    position: 'fixed',
                    pointerEvents: 'none',
                    left: rc.left + (rc.right - rc.left) * (pos2[0] / 2 + 0.5),
                    top:
                      rc.top + (rc.bottom - rc.top) * (-pos2[1] / 2 + 0.5) - 20,
                  });
                  if (div.last != e) {
                    div.innerHTML = e;
                    div.last = e;
                    if (self.renderMathInElement)
                      self.renderMathInElement(div, { output: 'html' });
                  }
                } else {
                  gl.enable(gl.BLEND);
                  gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        ...
        canvas.oncontextmenu = canvas.onmousedown = (e) => {
          ...
          canvas.ontouchmove = function (e) {
            ...
            options.p = Math.max(
              -Math.PI / 2,
              Math.min(Math.PI / 2, (options.p || 0) + my)
            );
            if (!options.animate)
              requestAnimationFrame(canvas.update.bind(canvas, f, options));
            return;
          ...
          canvas.onmousemove = (e) => {
            ...
            if (sel == -2) {
              options.h =
                (options.h || 0) + ((options.conformal ? -1 : 1) * mx) / 2;
              options.p = Math.max(
                -Math.PI / 2,
                Math.min(Math.PI / 2, (options.p || 0) - my / 2)
              );
              if (options.camera)
                options.camera.set(
                  Element.Bivector(0, 0, 0, 0, 0, options.p)
                    .Exp()
                    .Mul(Element.Bivector(0, 0, 0, 0, options.h, 0).Exp())
                );
              if (!options.animate)
                requestAnimationFrame(canvas.update.bind(canvas, f, options));
              return;
            ...
            if (tot == 5) {
              x.pos[0] +=
                e.buttons != 2
                  ? Math.cos(options.h || 0) * mx
                  : Math.sin(-(options.h || 0)) * -my;
              x.pos[1] += e.buttons != 2 ? -my : 0;
              x.pos[2] +=
                e.buttons != 2
                  ? Math.sin(options.h || 0) * mx
                  : Math.cos(-(options.h || 0)) * -my;
              canvas.value[sel].set(
                Element.Mul(
                  ni,
                  (x.pos[0] ** 2 + x.pos[1] ** 2 + x.pos[2] ** 2) * 0.5
                ).Sub(no)
              );
              ...
              canvas.value[sel].set(
                Element.sw(
                  Element.sw(
                    options.camera.Reverse,
                    Element.Bivector(-mx / 2, my / 2, 0, 0, 0, 0).Exp()
                  ),
                  canvas.value[sel]
                )
              );
      ...
      static inline(intxt) {
        ...
        if (arguments.length > 1 || intxt instanceof Array) {
          ...
          return res
            .inline(
              new Function(
                args.map((x, i) => '_template_' + i).join(),
                'return (' +
                  intxt
                    .map(
                      (x, i) =>
                        (x || '') + ((args[i] && '_template_' + i) || '')
                    )
                    .join('') +
                  ')'
              )
        ...
        tok = tok.map((t) =>
          t[0] == 2
            ? [
                ...
                'Element.Coeff(' +
                  basis.indexOf(
                    (!options.Cayley ? simplify : (x) => x)(
                      'e' + t[1].split(/e_|e|i/)[1] || 1
                    ).replace('-', '')
                  ) +
                  ',' +
                  (simplify(t[1].split(/e_|e|i/)[1] || 1).match('-')
                    ? '-1*'
                    : '') +
        ...
        tok = tok.map((t) =>
          t[0] != 6
            ? t
            : [].concat.apply([], syntax).filter((x) => x[0] == t[1]).length
            ? [6, [].concat.apply([], syntax).filter((x) => x[0] == t[1])[0][1]]
            : t
        );
        ...
        function translate(tokens) {
          ...
          var left = (x = ti - 1, skip = [0]) => {
            ...
            glue = (x, y, tp = 6, sub) => {
              tokens.splice(x, y - x + 1, [
                tp,
                ...(sub || tokens.slice(x, y + 1)),
              ]);
            },
          ...
          for (var ti = 0, t, si; (t = tokens[ti]); ti++)
            if (t[1] == '(')
              glue(ti, (si = match()), 7, [
                [5, '('],
                ...translate(tokens.slice(ti + 1, si)),
                [5, ')'],
              ]);
          // [] dot call and new
          for (var ti = 0, t, si; (t = tokens[ti]); ti++) {
          ...
          for (var ti = 0, t; (t = tokens[ti]); ti++)
            if (t[1] == '++' || t[1] == '--') glue(left(), ti);
          // unary - and + are handled separately from syntax ..
          for (var ti = 0, t, si; (t = tokens[ti]); ti++)
            if (t[1] == '-' && (left() < 0 || (tokens[left()] || [5])[0] == 5))
              glue(ti, right(), 6, [
                'Element.Sub(',
                tokens[right()],
                ')',
          ...
          for (var si = 0, s; (s = syntax[si]); si++)
            for (
              var ti = s[0][3] ? tokens.length - 1 : 0, t;
              (t = tokens[ti]);
              s[0][3] ? ti-- : ti++
            )
        ...
        return eval(
          '(' +
            (function f(t) {
              return t
                .map((t) =>
                  t instanceof Array ? f(t) : typeof t == 'string' ? t : ''
                )
                .join('');
            })(translate(tok)) +
            ')'
    ...
    if ((p == 2 || p == 3) && r == 1) {
      res.arrow = res.inline((from_point, to_point, w = 0.03, aspect = 0.8) => {
        ...
        return (
          ((1 + from_point / !1).Normalized *
            (1 + ((to_point - from_point) & !1).Normalized / (!1e1 & !1))
              .Normalized) >>>
          shape
        );
      });
    ...
    if (options.dual) {
      ...
      Object.defineProperty(res.prototype, 'Inverse', {
        ...
        get() {
          ...
          if (tot > 5) {
            for (
              var N = 2 ** (((tot + 1) / 2) | 0), Uk = this.Scale(1), k = 1;
              k < N;
              ++k
            ) {
          ...
          return tot == 0
            ...
            : tot == 1
            ? this.Involute.Mul(
                this.constructor.Scalar(1 / this.Mul(this.Involute)[0])
              )
            : tot == 2
            ? this.Conjugate.Mul(
                this.constructor.Scalar(1 / this.Mul(this.Conjugate)[0])
              )
            : tot == 3
            ? this.Reverse.Mul(this.Involute)
                .Mul(this.Conjugate)
                .Mul(
                  this.constructor.Scalar(
                    1 /
            ...
            : tot == 4
            ? this.Conjugate.Mul(this.Mul(this.Conjugate).Map(3, 4)).Mul(
                this.constructor.Scalar(
                  1 /
                    this.Mul(this.Conjugate).Mul(
                      this.Mul(this.Conjugate).Map(3, 4)
                    )[0]
                )
              )
    ...
    if (options.over) {
      ...
      res.prototype.Coeff = function () {
        for (var i = 0, l = arguments.length; i < l; i += 2)
          this[arguments[i]] =
            arguments[i + 1] instanceof options.over
              ? arguments[i + 1]
              : options.over.Scalar(arguments[i + 1]);
        return this;
      };
      ...
      Object.defineProperty(res.prototype, 'Inverse', {
        ...
        get() {
          return tot == 0
            ...
            : tot == 1
            ? this.Involute.Mul(
                this.constructor.Scalar(this.Mul(this.Involute)[0].Inverse)
              )
            : tot == 2
            ? this.Conjugate.Mul(
                this.constructor.Scalar(this.Mul(this.Conjugate)[0].Inverse)
              )
            : tot == 3
            ? this.Reverse.Mul(this.Involute)
                .Mul(this.Conjugate)
      ...
      res.prototype.toString = function () {
        return [...this]
          .map((x, i) =>
            x == 0 ? undefined : i ? '(' + x + ')' + basis[i] : x.toString()
          )
          .filter((x) => x)
          .join(' + ');
      };
