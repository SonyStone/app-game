...
export class SVGLoader extends Loader {
  ...
  parse(text: string | ArrayBuffer) {
    ...
    function parseNode(
      ...
      switch (node.nodeName) {
        ...
        case 'style':
          parseCSSStylesheet(node as SVGStyleElement);
          break;
        ...
        case 'use':
          ...
          const usedNode = (node.viewportElement as any)?.getElementById(
            usedNodeId
          );
          if (usedNode) {
      ...
      if (path) {
        if (style.fill !== undefined && style.fill !== 'none') {
          path.color.setStyle(style.fill as string);
        }
    ...
    function parsePathNode(node: SVGElement) {
    ...
    function parseCSSStylesheet(node: SVGStyleElement) {
     ...
     */
    function parseRectNode(node: SVGRectElement) {
    ...
    function parsePolygonNode(node: SVGPolygonElement) {
      function iterator(match: any, a: string, b: string) {
    ...
    function parsePolylineNode(node: SVGPolylineElement) {
      function iterator(match: any, a: string, b: string) {
        const x = parseFloatWithUnits(a);
        const y = parseFloatWithUnits(b);
    ...
    function parseCircleNode(node: SVGCircleElement) {
    ...
    function parseEllipseNode(node: SVGEllipseElement) {
      const x = parseFloatWithUnits(node.getAttribute('cx') || 0);
      const y = parseFloatWithUnits(node.getAttribute('cy') || 0);
    ...
    function parseLineNode(node: SVGLineElement) {
    ...
    function parseStyle(node: SVGElement, style: { [key: string]: any }) {
      ...
      function addStyle(
        ...
        if (node.style && node.style[svgName as any] !== '')
          style[jsName] = adjustFunction(node.style[svgName as any]);
      }
    ...
    parseNode(xml.documentElement as any, {
      fill: '#000',
      fillOpacity: 1,
      strokeOpacity: 1,
      strokeWidth: 1,
  ...
  static createShapes(shapePath: ShapePath) {
    ...
    function getIntersections(path1: Vector2[], path2: Vector2[]) {
    ...
    let simplePaths: SimplePath[] = shapePath.subPaths.map((p: Path) => {
    ...
    const isAHole = simplePaths.map((p) =>
      isHoleTo(
        ...
        scanlineMinX,
        scanlineMaxX,
        (shapePath as any).userData.style.fillRule
      )
    );
  ...
  static pointsToStrokeWithBuffers(
    ...
    function addVertex(position: Vector2, u: any, v: any) {
      if (vertices) {
