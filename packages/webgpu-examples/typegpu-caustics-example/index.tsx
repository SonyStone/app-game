...
function TypeGPUApp() {
  ...
  onMount(() => {
    tileDensity.write(10);
  });

  return (
    <div class="flex flex-col place-items-start">
      <button class="rounded bg-blue-500 p-2 text-white" onClick={() => (running() ? stop() : start())}>
        {running() ? 'Stop' : 'Start'}
      </button>
      <input type="range" min="5" max="20" step="1" onInput={(e) => tileDensity.write(Number(e.currentTarget.value))} />
      {canvas}
    </div>
  );
}
