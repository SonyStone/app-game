use std::{f32::consts::PI, rc::Rc};

use fundamentals_common::{
    create_checker_texture,
    cube_data_old::get_cube_data,
    uniform::{SetUniform, Uniform},
};
use nalgebra::{Matrix4, Perspective3, Vector3};
use wasm_bindgen::prelude::*;
use web_sys::{
    console, WebGl2RenderingContext, WebGlProgram, WebGlSampler, WebGlTexture,
    WebGlVertexArrayObject,
};
use webgl_common::{create_shader_program, slice_as_u8_slice};

#[wasm_bindgen]
pub struct App {
    canvas: web_sys::HtmlCanvasElement,
    gl: Rc<WebGl2RenderingContext>,
    vao: WebGlVertexArrayObject,
    program: WebGlProgram,
    uniforms: Uniforms,
    checker_texture: WebGlTexture,
    linear_sampler: WebGlSampler,
    nearest_sampler: WebGlSampler,
}

struct Uniforms {
    projection: Uniform<Matrix4<f32>>,
    model_view: Uniform<Matrix4<f32>>,
    diffuse: Uniform<u32>,
    diffuse_mult: Uniform<[f32; 4]>,
    light_dir: Uniform<Vector3<f32>>,
}

#[wasm_bindgen]
impl App {
    /// is initialization code
    pub fn new(canvas: web_sys::HtmlCanvasElement) -> Self {
        let gl = canvas
            .get_context("webgl2")
            .unwrap()
            .unwrap()
            .dyn_into::<WebGl2RenderingContext>()
            .unwrap();

        let gl = Rc::new(gl);

        let program = create_shader_program(
            &gl,
            include_str!("shader.vert"),
            include_str!("shader.frag"),
        );

        let position_loc = gl.get_attrib_location(&program, "position") as u32;
        let normal_loc = gl.get_attrib_location(&program, "normal") as u32;
        let texcoord_loc = gl.get_attrib_location(&program, "texcoord") as u32;

        let (
            cube_vertex_positions,
            cube_vertex_normals,
            cube_vertex_texcoords,
            cube_vertex_indices,
        ) = get_cube_data();

        let cube_vertex_array = {
            let cube_vertex_array = gl.create_vertex_array().unwrap();
            gl.bind_vertex_array(Some(&cube_vertex_array));

            {
                let position_buffer = gl.create_buffer().unwrap();
                gl.bind_buffer(WebGl2RenderingContext::ARRAY_BUFFER, Some(&position_buffer));
                gl.buffer_data_with_u8_array(
                    WebGl2RenderingContext::ARRAY_BUFFER,
                    slice_as_u8_slice(&cube_vertex_positions),
                    WebGl2RenderingContext::STATIC_DRAW,
                );
                gl.enable_vertex_attrib_array(position_loc);
                gl.vertex_attrib_pointer_with_i32(
                    position_loc,                  // location
                    3,                             // size (components per iteration)
                    WebGl2RenderingContext::FLOAT, // type of to get from buffer
                    false,                         // normalize
                    0,                             // stride (bytes to advance each iteration)
                    0,                             // offset (bytes from start of buffer)
                );
            }

            {
                let normal_buffer = gl.create_buffer().unwrap();
                gl.bind_buffer(WebGl2RenderingContext::ARRAY_BUFFER, Some(&normal_buffer));
                gl.buffer_data_with_u8_array(
                    WebGl2RenderingContext::ARRAY_BUFFER,
                    slice_as_u8_slice(&cube_vertex_normals),
                    WebGl2RenderingContext::STATIC_DRAW,
                );
                gl.enable_vertex_attrib_array(normal_loc);
                gl.vertex_attrib_pointer_with_i32(
                    normal_loc,                    // location
                    3,                             // size (components per iteration)
                    WebGl2RenderingContext::FLOAT, // type of to get from buffer
                    false,                         // normalize
                    0,                             // stride (bytes to advance each iteration)
                    0,                             // offset (bytes from start of buffer)
                );
            }

            {
                let texcoord_buffer = gl.create_buffer().unwrap();
                gl.bind_buffer(WebGl2RenderingContext::ARRAY_BUFFER, Some(&texcoord_buffer));
                gl.buffer_data_with_u8_array(
                    WebGl2RenderingContext::ARRAY_BUFFER,
                    slice_as_u8_slice(&cube_vertex_texcoords),
                    WebGl2RenderingContext::STATIC_DRAW,
                );
                gl.enable_vertex_attrib_array(texcoord_loc);
                gl.vertex_attrib_pointer_with_i32(
                    texcoord_loc,                  // location
                    2,                             // size (components per iteration)
                    WebGl2RenderingContext::FLOAT, // type of to get from buffer
                    false,                         // normalize
                    0,                             // stride (bytes to advance each iteration)
                    0,                             // offset (bytes from start of buffer)
                );
            }

            {
                let index_buffer = gl.create_buffer().unwrap();
                gl.bind_buffer(
                    WebGl2RenderingContext::ELEMENT_ARRAY_BUFFER,
                    Some(&index_buffer),
                );
                gl.buffer_data_with_u8_array(
                    WebGl2RenderingContext::ELEMENT_ARRAY_BUFFER,
                    slice_as_u8_slice(&cube_vertex_indices),
                    WebGl2RenderingContext::STATIC_DRAW,
                );
            }

            // This is not really needed but if we end up binding anything
            // to ELEMENT_ARRAY_BUFFER, say we are generating indexed geometry
            // we'll change cubeVertexArray's ELEMENT_ARRAY_BUFFER. By binding
            // null here that won't happen.
            gl.bind_vertex_array(None);

            cube_vertex_array
        };

        let linear_sampler = {
            let linear_sampler = gl.create_sampler().unwrap();
            gl.sampler_parameteri(
                &linear_sampler,
                WebGl2RenderingContext::TEXTURE_MIN_FILTER,
                WebGl2RenderingContext::LINEAR as i32,
            );
            linear_sampler
        };
        let nearest_sampler = {
            let nearest_sampler = gl.create_sampler().unwrap();
            gl.sampler_parameteri(
                &nearest_sampler,
                WebGl2RenderingContext::TEXTURE_MIN_FILTER,
                WebGl2RenderingContext::NEAREST as i32,
            );
            gl.sampler_parameteri(
                &nearest_sampler,
                WebGl2RenderingContext::TEXTURE_MAG_FILTER,
                WebGl2RenderingContext::NEAREST as i32,
            );
            nearest_sampler
        };

        Self {
            uniforms: Uniforms {
                projection: Uniform::new(
                    Rc::clone(&gl),
                    gl.get_uniform_location(&program, "projection").unwrap(),
                ),
                model_view: Uniform::new(
                    Rc::clone(&gl),
                    gl.get_uniform_location(&program, "modelView").unwrap(),
                ),
                diffuse: Uniform::new(
                    Rc::clone(&gl),
                    gl.get_uniform_location(&program, "diffuse").unwrap(),
                ),
                diffuse_mult: Uniform::new(
                    Rc::clone(&gl),
                    gl.get_uniform_location(&program, "diffuseMult").unwrap(),
                ),
                light_dir: Uniform::new(
                    Rc::clone(&gl),
                    gl.get_uniform_location(&program, "lightDir").unwrap(),
                ),
            },
            checker_texture: create_checker_texture(&gl),
            canvas,
            vao: cube_vertex_array,
            program,
            gl,
            linear_sampler,
            nearest_sampler,
        }
    }

    /// is rendering code.
    pub fn render(&self) {
        {
            // Set up WebGL state before rendering
            // and cleanup the canvas
            self.gl.viewport(
                0,
                0,
                self.canvas.width() as i32,
                self.canvas.height() as i32,
            );

            self.gl.clear_color(0.5, 0.7, 1.0, 1.0);
            self.gl.clear(
                WebGl2RenderingContext::COLOR_BUFFER_BIT | WebGl2RenderingContext::DEPTH_BUFFER_BIT,
            );

            self.gl.enable(WebGl2RenderingContext::DEPTH_TEST);
            self.gl.enable(WebGl2RenderingContext::CULL_FACE);
        }

        {
            self.gl.use_program(Some(&self.program));
            self.gl.bind_vertex_array(Some(&self.vao));
            let tex_unit = {
                // Picking unit 6 just to be different. The default of 0
                // would render but would show less state changing.
                let tex_unit = 6;
                self.gl
                    .active_texture(WebGl2RenderingContext::TEXTURE0 + tex_unit);
                self.gl.bind_texture(
                    WebGl2RenderingContext::TEXTURE_2D,
                    Some(&self.checker_texture),
                );
                self.uniforms.diffuse.set(&tex_unit);
                tex_unit
            };
            {
                // set up light direction
                self.uniforms
                    .light_dir
                    .set(&Vector3::new(1.0, 5.0, 8.0).normalize());
            }
            {
                // set up projection matrix
                let projection = Perspective3::new(
                    self.canvas.client_width() as f32 / self.canvas.client_height() as f32,
                    PI / 4.0,
                    0.1,
                    10.0,
                );

                self.uniforms.projection.set(projection.as_matrix());
            }
            {
                // draw left cube

                let model_view = Matrix4::new_translation(&Vector3::new(-2.0, 0.0, -4.0))
                    * Matrix4::from_euler_angles(0.5, 0.0, 0.0)
                    * Matrix4::from_euler_angles(0.0, 0.5, 0.0);

                self.uniforms.model_view.set(&model_view);
                self.uniforms.diffuse_mult.set(&[0.7, 1.0, 0.7, 1.0]);

                self.gl.bind_sampler(tex_unit, Some(&self.linear_sampler));

                self.gl.draw_elements_with_i32(
                    WebGl2RenderingContext::TRIANGLES,
                    36,                                     // num vertices to process
                    WebGl2RenderingContext::UNSIGNED_SHORT, // type of indices
                    0,                                      // offset on bytes to indices
                );
            }

            {
                // draw right cube

                let model_view = Matrix4::new_translation(&Vector3::new(2.0, 0.0, -4.0))
                    * Matrix4::from_euler_angles(0.5, 0.0, 0.0)
                    * Matrix4::from_euler_angles(0.0, 0.8, 0.0);

                self.uniforms.model_view.set(&model_view);
                self.uniforms.diffuse_mult.set(&[1.0, 0.7, 0.7, 1.0]);

                self.gl.bind_sampler(tex_unit, Some(&self.nearest_sampler));

                self.gl.draw_elements_with_i32(
                    WebGl2RenderingContext::TRIANGLES,
                    36,                                     // num vertices to process
                    WebGl2RenderingContext::UNSIGNED_SHORT, // type of indices
                    0,                                      // offset on bytes to indices
                );
            }
        }
    }
}
