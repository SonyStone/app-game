...
import { Id, Listeners, Transformer, useDragDropContext } from './drag-drop-context';
import { useSortableContext } from './sortable-context';
import { RefSetter, combineRefs } from './utils/combine-refs';
import { Layout, Transform, isTransformsAreEqual, noopTransform } from './utils/layout';
...
export const createSortable = (id: Id, data: Record<string, any> = {}): Sortable => {
  ...
  const layoutById = (id: Id): Layout | null => dndState.droppables[id]?.layout || null;
  ...
  onSettled(() => addTransformer('droppables', id, transformer));
