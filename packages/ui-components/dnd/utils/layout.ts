...
export const elementLayout = (element: HTMLElement): Layout => {
...
export const stripTransformFromLayout = (layout: Layout, transform: string): Layout => {
...
/**
 * Zero Transform `{ x: 0, y: 0 }`
 */
export const noopTransform = (): Transform => ({ x: 0, y: 0 })

/**
 * Check is both Transforms are Equal
 * `x === x && y === y`
 */
export const isTransformsAreEqual = (
...
/**
 * creates new Layout from old Layout and Transform
 */
export const transformLayout = (layout: Layout, transform: Transform): Layout => {
  return new Layout({
    ...layout,
...
export const distanceBetweenPoints = (firstPoint: Point, secondPoint: Point): number => {
  return Math.sqrt(
    Math.pow(firstPoint.x - secondPoint.x, 2) + Math.pow(firstPoint.y - secondPoint.y, 2),
  )
}

export const intersectionRatioOfLayouts = (firstLayout: Layout, secondLayout: Layout): number => {
...
export const layoutsAreEqual = (firstLayout: Layout, secondLayout: Layout): boolean => {
  return (
...
export const layoutContainsPoint = (layout: Layout, point: Point): boolean => {
  return !(
    point.x < layout.left ||
