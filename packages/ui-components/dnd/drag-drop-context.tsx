...
import { elementLayout, Layout, layoutsAreEqual, noopTransform, Transform, transformLayout } from './utils/layout';
...
export type Listeners = Record<string, (event: HTMLElementEventMap[keyof HTMLElementEventMap]) => void>;
...
export const [DragDropProvider, useDragDropContext] = createContextProvider(
  (passedProps: {
    ...
    const addTransformer = (type: 'draggables' | 'droppables', id: Id, transformer: Transformer): void => {
    ...
    const removeTransformer = (type: 'draggables' | 'droppables', id: Id, transformerId: Id): void => {
      ...
      if (!untrack(() => state[type][id]?.['transformers'][transformerId])) {
        console.warn(
          `Cannot remove from ${toDisplay(type)} with id ${id}, nonexistent transformer with id: ${transformerId}`
        );
        return;
      }
      ...
      if (!existing) {
        Object.defineProperties(overlay, {
          ...
          transformers: {
            ...
            get: () =>
              Object.fromEntries(
                Object.entries(state.active.draggable ? state.active.draggable.transformers : {}).filter(
                  ([id]) => id !== 'addDraggable-existing-offset'
                )
              )
          },
          transform: {
    ...
    const onDragEnd = (handler: DragEventHandler): void => {
      createEffect(
        ...
        ({ draggable }, previous) => {
          if (!draggable && previous?.draggable) {
            untrack(() =>
              handler({
