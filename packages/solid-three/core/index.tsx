...
import { createThreeStore, isRenderer, Renderer, RootState, Size, StoreProps, ThreeContext, ThreeStore } from './store';
import { applyProps, dispose } from './utils';

export type { Camera, EventHandlers, Intersection, IntersectionEvent } from './events';
...
export type RenderProps<TCanvas extends Element> = Omit<StoreProps, 'gl' | 'events' | 'size'> & {
  gl?: GLProps;
  events?: (store: ThreeStore) => EventManager<TCanvas>;
  size?: Size;
...
const createRendererInstance = <TElement extends Element>(gl: GLProps, canvas: TElement): THREE.WebGLRenderer => {
  ...
  (renderer as THREE.WebGLRenderer & { outputColorSpace: unknown }).outputColorSpace = (THREE as unknown as {
...
function createThreeRoot<TCanvas extends HTMLElement>(canvas: TCanvas, config?: RenderProps<TCanvas>) {
