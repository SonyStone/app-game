...
import type { ThreeStore } from './store';
...
export function removeInteractivity(store: ThreeStore, object: THREE.Object3D) {
  const [state] = store;
...
export function createEvents(store: ThreeStore) {
  ...
  function patchIntersects(intersections: Intersection[], event: DomEvent) {
    ...
    if ('pointerId' in event && internal.capturedMap.has(event.pointerId)) {
      for (const captureData of internal.capturedMap.get(event.pointerId)!.values()) {
  ...
  function handleIntersects(
    ...
    const [state] = store;
    const { raycaster, mouse, camera, internal } = state;
  ...
  const handlePointer = (name: string) => {
    ...
    return (event: DomEvent) => {
      const [state] = store;
      const { onPointerMissed, internal } = state;
  ...
  function pointerMissed(event: MouseEvent, objects: THREE.Object3D[]) {
    objects.forEach(
      (object: THREE.Object3D) => (object as unknown as Instance).__r3f?.handlers.onPointerMissed?.(event)
    );
  }
