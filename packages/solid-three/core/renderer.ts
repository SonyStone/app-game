...
import { applyProp, applyProps, attach, detach, invalidateInstance, prepare, updateInstance } from './utils';

export type Root = { store: ThreeStore };
...
const isStore = (def: any): def is ThreeStore => def && Array.isArray(def) && def.length === 2;
// const getContainer = (
//   container: ThreeStore | Instance,
//   child: Instance
// ) => ({
...
function createThreeRenderer<TCanvas>(roots: Map<TCanvas, Root>, getEventPriority?: () => any) {
  function createInstance(type: string, { args = [], attach, ...props }: InstanceProps, root: ThreeStore | Instance) {
    ...
    //   const fn = (node: Reconciler.Fiber): ThreeStore => {
  ...
  function removeChild(parentInstance: Instance, child: Instance, canDispose?: boolean) {
    if (child) {
      ...
      const shouldDispose = canDispose === undefined ? child.dispose !== null && !isPrimitive : canDispose;
...
export { createThreeRenderer, extend, prepare };
