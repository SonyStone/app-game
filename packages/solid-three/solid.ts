...
export function log(renderer: string, action: any, ...options: any[]) {
  DEBUG && console.debug(`%c${renderer} %s  %c`, 'font-weight: bold', action, 'font-weight: normal', ...options);
}
...
export function createSolidRenderer({
  ...
  return createRenderer<Instance>({
    ...
    setProperty(node: Instance, key: string, value: any) {
      ...
      if (/^on(Pointer|Click|DoubleClick|ContextMenu|Wheel)/.test(key)) return applyProp(node, [key, value, true, []]);
      // Split dashed props
      let entries: string[] = [];
