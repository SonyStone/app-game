...
export default function VirtualScrollNestedExample(props: { embedded?: boolean } = {}) {
  ...
  return (
    <div class={props.embedded ? '' : 'min-h-screen bg-zinc-50 p-4 text-zinc-950'}>
      <div class="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(22rem,0.75fr)]">
        <section class="flex h-[820px] min-w-0 flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white">
...
function VirtualTree(props: {
  ...
  const virtual = createVirtualNestedList({
    ...
    elementRef: scroller,
    estimateOwnHeight: (item) => Object.keys(readStringProperties(item)).length * 54 + 48,
    overscan: 800,
    gap: 8
  });
...
function RenderedSvg(props: { items: readonly NestedItem[] }) {
  return (
    <section class="flex h-[820px] min-h-0 flex-col overflow-hidden rounded-lg border border-zinc-200 bg-white">
      ...
      <div class="grid min-h-0 flex-1 place-items-center overflow-hidden bg-zinc-50 p-4">
        <div class="aspect-square h-full max-h-full max-w-full border border-zinc-200 bg-white [&>svg]:h-full [&>svg]:w-full">
