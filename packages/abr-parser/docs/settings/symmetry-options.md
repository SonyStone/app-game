...
Symmetry allows painting with automatic mirrored/repeated strokes based on various symmetry patterns. This is **tool metadata** - symmetry settings are runtime preferences, not stored in ABR brush definitions.

> **Note:** Symmetry is NOT part of ABR brush data. It's a runtime tool setting controlled via the canvas. This document is included for completeness as it affects brush behavior in editors.
...
- Uses currently selected vector path as symmetry axis
- Use Case: Custom symmetry shapes

### Transform Symmetry

- Opens transform controls to move/rotate/scale symmetry axes
- Allows repositioning the center point and axes

### Hide Symmetry

- Hides the symmetry guides while keeping symmetry active
...
```typescript
function getRadialPoints(x: number, y: number, centerX: number, centerY: number, segments: number) {
  const points = [];
