import { Archetype, InternalArchetype, createArchetype, transformArchetype, traverseArchetypeGraph } from './archetype';
import { createBitSet } from './collections/bit-set';
...
export class World<TUpdateArguments extends any[] = never[]> implements OutsideWorld<TUpdateArguments>, InsideWorld {
  ...
  createEntity(prefabricate: Archetype = this.rootArchetype): number {
    const entity = this.deletedEntities.length > 0 ? this.deletedEntities.pop()! : this.nextEntityId++;
  ...
  hasComponent<T extends Component>(entity: number, component: T): boolean {
    return (
      this.entityArchetype[entity] !== undefined && this.entityArchetype[entity]!.mask.has(getComponentId(component))
    );
  }
