...
const fetch = <T = any>(path: string): T =>
...
function zip<A, B>(a: A[], b: B[]) {
  ...
  for (
    let i = 0, len = a.length > b.length ? a.length : b.length;
    i < len;
    ++i
  ) {
      ...
      switch (true) {
        case T("Point"):
          o_val == null
            ...
            : (expect(o_val.cx).toEqual(p_val.x),
              expect(o_val.cy).toEqual(p_val.y));
          break;
        case T("Enum"):
          expect(T((p as EnumField).ref.id)).toEqual(true);
      }
...
describe("importer", () => {
  it("full API", () => {
    ...
    for (
      let tilesetIdx = 0;
      tilesetIdx < data.defs!.tilesets.length;
      ++tilesetIdx
    ) {
    ...
    for (let levelIdx = 0; levelIdx < data.levels.length; ++levelIdx) {
      ...
      if (o_level.layerInstances != null) {
        ...
        for (
          let layerIdx = 0;
          layerIdx < o_level.layerInstances.length;
          ++layerIdx
        ) {
          ...
          if (o_layer.__type === "AutoLayer") {
            ...
            expect(o_layer.entityInstances.length).toEqual(
              p_layer.entities!.length
            );
            for (
              let entityIdx = 0;
              entityIdx < o_layer.entityInstances.length;
              ++entityIdx
            ) {
              const o_entity = o_layer.entityInstances[entityIdx];
              const p_entity = p_layer.entities![entityIdx];
              expect(o_entity.fieldInstances.length).toEqual(
                Object.keys(p_entity.fields).length
              );
            ...
            for (
              let intGridIdx = 0;
              intGridIdx < o_layer.intGrid.length;
              ++intGridIdx
            ) {
