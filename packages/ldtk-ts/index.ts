...
function parseField(
  ...
  if (result == null)
    ...
    throw new Error(
      `Error at field '${field.__identifier}' entity '${entityId}': Invalid type name ${field.__type}`
    );
...
export class Entity {
  ...
  constructor(
    ...
    for (let i = 0; i < data.fieldInstances.length; ++i) {
      ...
      (this.fields as Record<string, Field>)[instance.__identifier] =
        parseField(instance, world, this.id);
    }
...
export class Layer {
  ...
  constructor(public readonly world: World, private data: LDtk.LayerInstance) {
...
export class Level {
  ...
  constructor(public readonly world: World, private data: LDtk.Level) {
    this.background = new Background(data);
    if (this.data.layerInstances != null) {
      for (let i = 0; i < this.data.layerInstances.length; ++i) {
        (this.layers as Array<Layer>)[i] = new Layer(
          world,
          this.data.layerInstances[i]
        );
      }
    }
  }
  ...
  get neighbours(): Neighbour[] {
    if (this.neighbours_ == null) {
      ...
      for (let i = 0; i < this.data.__neighbours.length; ++i) {
        const ref = this.world.findLevelByUid(
          this.data.__neighbours[i].levelUid
        );
        if (ref == null)
          // sanity check, should never happen
          throw new Error(
            `Neighbour '${this.data.__neighbours[i].levelUid}' for level '${this.data.identifier}' does not exist.`
          );
...
export class Enum {
  ...
  constructor(public readonly world: World, private data: LDtk.EnumDefinition) {
    this.valueMap = {};
...
export class World {
  ...
  private constructor(private data: LDtk.World) {
    ...
    for (let i = 0; i < data.defs.tilesets.length; ++i) {
      ...
      (this.tilesetMap as Record<string, Tileset>)[t.identifier] = new Tileset(
        this,
        t
      );
    }
    ...
    if (!data.externalLevels) {
      ...
      for (let i = 0; i < data.levels.length; ++i) {
        (this.levelMap as Record<string, Level>)[data.levels[i].identifier] =
          new Level(this, data.levels[i]);
      }
  ...
  async loadLevels(): Promise<void> {
    ...
    for (let i = 0; i < levels.length; ++i) {
      ...
      if (rel != null) {
        promises.push(
          this.fetchLevel(rel).then((loaded) => {
  ...
  async loadLevel(identifier: string): Promise<void> {
    ...
    let levelIndex = -1;
    let level: LDtk.Level | null = null;
