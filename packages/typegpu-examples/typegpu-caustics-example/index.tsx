...
export default function TypeGPUExample() {
  ...
  const [tileDensity, setTileDensity] = createSignal(10);
    ...
    <div class="flex flex-col place-items-start">
      ...
      <div class="grid w-full grid-cols-[200px_minmax(900px,_1fr)] gap-4 px-8">
        ...
        <input
          ...
          step="1"
          value={tileDensity()}
          onInput={(e) => setTileDensity(Number(e.currentTarget.value))}
        />
        <pre>speed: {speed()}</pre>
        <input
          class=" flex-grow"
      ...
      <TypeGPUProvider style={{ width: `${width()}px`, height: `${height()}px` }} class="border-2 border-black">
        <App tileDensity={tileDensity()} speed={speed()} running={running()} />
      </TypeGPUProvider>
    </div>
  );
}

function App(props: { tileDensity: number; running: boolean; speed: number }) {
