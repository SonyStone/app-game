 ...
 */
export class LoaderPlugin extends Phaser.Events.EventEmitter {
  constructor(scene: Scene) {
    super();

    const gameConfig = scene.sys.game.config;
    const sceneConfig = scene.sys.settings.loader;
    ...
    this.setBaseURL(sceneConfig.baseURL ?? gameConfig.loaderBaseURL);

    this.setPath(sceneConfig.path ?? gameConfig.loaderPath);

    this.setPrefix(sceneConfig.prefix ?? gameConfig.loaderPrefix);

    this.maxParallelDownloads =
      sceneConfig.maxParallelDownloads ?? gameConfig.loaderMaxParallelDownloads;

    this.xhr = XHRSettings(
      sceneConfig.responseType ?? gameConfig.loaderResponseType,
      sceneConfig.async ?? gameConfig.loaderAsync,
      sceneConfig.user ?? gameConfig.loaderUser,
      sceneConfig.password ?? gameConfig.loaderPassword,
      sceneConfig.timeout ?? gameConfig.loaderTimeout,
      sceneConfig.withCredentials ?? gameConfig.loaderWithCredentials
    );

    this.crossOrigin = sceneConfig.crossOrigin ?? gameConfig.loaderCrossOrigin;
    this.imageLoadType =
      sceneConfig.imageLoadType ?? gameConfig.loaderImageLoadType;
    this.localSchemes = sceneConfig.localScheme ?? gameConfig.loaderLocalScheme;
   ...
   */
  list = new Set<Phaser.Loader.File>();
   ...
   * By the end of the load process this Set will be empty.
   */
  inflight = new Set<Phaser.Loader.File>();
   ...
   *
   * @return `true` if adding this file will cause a cache or queue conflict, otherwise `false`.
   */
  keyExists(file: File): boolean {
    let keyConflict = file.hasCacheConflict();
  ...
  addPack(pack: any, packKey: string) {
    ...
    for (const key in pack) {
      ...
      const config = pack[key];

      //  Any meta data to process?
      const baseURL = config.baseURL ?? currentBaseURL;
      const path = config.path ?? currentPath;
      const prefix = config.prefix ?? currentPrefix;
      const files = config.files ?? null;
      const defaultType = config.defaultType ?? 'void';
      ...
      if (Array.isArray(files)) {
        ...
        for (let i = 0; i < files.length; i++) {
          const file = files[i];
          const type = file.hasOwnProperty('type') ? file.type : defaultType;

          if ((this as any)[type]) {
            (this as any)[type](file);
            total++;
          }
        }
      }
  ...
  isLoading() {
    return (
      this.state === FILE_CONST.LOADER_LOADING ||
      this.state === FILE_CONST.LOADER_PROCESSING
    );
  }
  ...
  isReady() {
    return (
      this.state === FILE_CONST.LOADER_IDLE ||
      this.state === FILE_CONST.LOADER_COMPLETE
    );
  }
  ...
  checkLoadQueue() {
    this.list.forEach((file) => {
      if (
        file.state === FILE_CONST.FILE_POPULATED ||
        (file.state === FILE_CONST.FILE_PENDING &&
          this.inflight.size < this.maxParallelDownloads)
      ) {
   ...
   */
  nextFile(file: Phaser.Loader.File, success: boolean) {
   ...
   */
  fileProcessComplete(file: Phaser.Loader.File) {
    //  Has the game been destroyed during load? If so, bail out now.
    if (
      !this.scene ||
      !this.systems ||
    ...
    if (file.state === FILE_CONST.FILE_ERRORED) {
      if (file.multiFile) {
        file.multiFile.onFileFailed(file);
      }
    } else if (file.state === FILE_CONST.FILE_COMPLETE) {
      if (file.multiFile) {
  ...
  loadComplete() {
    ...
    this._deleteQueue.forEach((item) => item.destroy());
   ...
   */
  flagForRemoval(file: Phaser.Loader.File) {
    this._deleteQueue.add(file);
  }
   ...
   */
  saveJSON(data: any, filename: string = 'file.json') {
  ...
  reset() {
    ...
    const gameConfig = this.systems.game.config;
    const sceneConfig = this.systems.settings.loader;
  ...
  shutdown() {
    ...
    this.state = FILE_CONST.LOADER_SHUTDOWN;
  ...
  destroy() {
    ...
    this.state = FILE_CONST.LOADER_DESTROYED;

    this.systems.events.off(SceneEvents.UPDATE, this.update, this);
    this.systems.events.off(SceneEvents.START, this.pluginStart, this);

    (this as any).list = null;
    (this as any).inflight = null;
    (this as any).queue = null;

    (this as any).scene = null;
    (this as any).systems = null;
    (this as any).textureManager = null;
    (this as any).cacheManager = null;
    (this as any).sceneManager = null;
  }
}
