...
export class JSONFile extends File {
  ...
  constructor(
    loader: LoaderPlugin,
    key: string | Phaser.Types.Loader.FileTypes.JSONFileConfig,
    url?: object | string,
    ...
    if (typeof key !== 'string') {
      ...
      key = config.key;
      url = config.url;
      xhrSettings = config.xhrSettings ?? xhrSettings;
      extension = config.extension ?? extension;
      dataKey = config.dataKey ?? dataKey;
    }
  ...
  onProcess() {
    if (this.state !== CONST.FILE_POPULATED) {
      ...
      try {
        const json = JSON.parse(this.xhrLoader.responseText);
      } catch (e) {
        this.onProcessError();
 ...
 * const data = this.cache.json.get('wavedata');
 * ```
 *
