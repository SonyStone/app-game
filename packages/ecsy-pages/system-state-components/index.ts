import { Entity, Not, Read, System, SystemData, SystemStateComponent, World } from '@packages/ecsy';
...
class MainSystem implements System {
  run(added: [Entity, Sprite][], removed: [Entity, SpriteResources], normal: [Entity, Sprite, SpriteResources]) {
    for (const [entity] of added) {
      ...
      log(
        `Added SpriteResources component: 'memory position: ${memPosition}, size: ${size}kb' to entity id=${entity.id}`
      );
...
world.registerComponent(Sprite).registerComponent(SpriteResources).registerSystem(MainSystem);
