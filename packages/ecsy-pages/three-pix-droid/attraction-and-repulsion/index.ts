    ...
    for (let i = 1; i < dots.length; i++) {
      ...
      for (let j = 0; j < dots.length; j++) {
        ...
        if (j === 0) {
          ...
          dist < config.mouseSize ? (force = (dist - config.mouseSize) * dotB.mass) : (force = dotA.mass);
        }
...
class AddDotsSystem implements System {
  run([mouseClick]: MouseClick[], [mouse]: MousePosition[]) {
    ...
    if (down) {
      world.createEntity().addComponent(Dot, { pos: { x: mouse.x, y: mouse.y } });
    }
  }
}
...
world.createEntity().addComponent(Dot, { rad: config.bigDotRad });
