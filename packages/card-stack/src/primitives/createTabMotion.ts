...
export function createTabMotion(
...
): Accessor<ReadonlyMap<string, number>> & { rebase: (painted: ReadonlyMap<string, number>) => void } {
  ...
  function publish() {
    ...
    if (previous.size === current.size && [...current].every(([id, value]) => previous.get(id) === value.x)) return;
