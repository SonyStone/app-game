...
import { IResultViewMenuState, MenuStatus, ResultViewMenuComponent } from './menu/resultViewMenuComponent';
import { ResultViewComponent } from './resultViewComponent';
import { ISourceCodeState, SourceCodeComponent } from './sourceCode/sourceCodeComponent';
import { VisualStateListComponent } from './visualStateList/visualStateListComponent';
import { IVisualStateItem, VisualStateListItemComponent } from './visualStateList/visualStateListItemComponent';
...
export class ResultView {
  ...
  private displayInformation(): void {
    ...
    this.displayJSONGroup(
      rightJsonContentStateId,
  ...
  private getJSONAsString(parentGroupId: number, key: string, json: any): string | null {
    if (json === null) {
      return 'null';
    }
  ...
  private displayCurrentCapture(): void {
    ...
    this.mvx.updateAllChildrenGenericState<ICaptureListItemState>(this.captureListStateId, (state) => {
      state.active = false;
      return state;
    });
  ...
  private displayCurrentCommand(): number {
    ...
    this.mvx.updateAllChildrenGenericState<ICommandListItemState>(this.commandListStateId, (state) => {
      state.active = false;
      return state;
    });
  ...
  private displayCurrentVisualState(): number | null {
    if (this.mvx.getGenericState<IResultViewMenuState>(this.menuStateId).status !== MenuStatus.Commands) {
      return null;
    }
    ...
    this.mvx.updateAllChildrenGenericState<IVisualStateItem>(this.visualStateListStateId, (state) => {
      state.active = false;
      return state;
    });
  ...
  private createVisualStates(capture: ICapture): void {
    ...
    this.initVisualStateId = this.mvx.addChildState(
      this.visualStateListStateId,
      {
  ...
  private createCommands(capture: ICapture): void {
    ...
    for (let i = 0; i < capture.commands.length; i++) {
      ...
      if (
        this.toFilter(commandCapture.marker) &&
      ...
      const commandStateId = this.mvx.addChildState(
        this.commandListStateId,
        commandState,
      ...
      if (commandCapture.VisualState) {
        ...
        tempVisualStateId = this.mvx.addChildState(
          this.visualStateListStateId,
          visualState,
          this.visualStateListItemComponent
        );
      ...
      if ((this.currentCommandId === -1 && i === 0) || this.currentCommandId === commandCapture.id) {
        this.currentCommandStateId = commandStateId;
        this.displayCurrentCommand();
