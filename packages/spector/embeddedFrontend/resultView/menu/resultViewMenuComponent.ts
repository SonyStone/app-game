    ...
    const htmlString = this.htmlTemplate`<ul class="resultViewMenuComponent">
                ...
                <li class="searchContainer">
                    <input type="text" placeHolder="Search..." value="${
                      state.searchText
                    }" commandName="onSearchTextChanged" commandEventBinding="change">
                    <a class="clearSearch" stoppropagation="true" CommandName="onSearchTextCleared">X</a>
                </li>
                <li><a class="${
                  state.status === MenuStatus.Captures ? 'active' : ''
                } href="#" role="button" commandName="onCapturesClicked">Captures</a></li>
                <li><a class="${
                  state.status === MenuStatus.Information ? 'active' : ''
                } href="#" role="button" commandName="onInformationClicked">Information</a></li>
                <li><a class="${
                  state.status === MenuStatus.InitState ? 'active' : ''
                } href="#" role="button" commandName="onInitStateClicked">Init State</a></li>
                <li>
                    <a class="${
                      state.status === MenuStatus.Commands ? 'active' : ''
                    } href="#" role="button" commandName="onCommandsClicked">
                        Commands${state.commandCount > 0 ? ' (' + state.commandCount + ')' : ''}
                    </a>
                </li>
                <li><a class="${
                  state.status === MenuStatus.EndState ? 'active' : ''
                } href="#" role="button" commandName="onEndStateClicked">End State</a></li>
