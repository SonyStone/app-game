...
import { IContextInformation, WebGLRenderingContexts } from '../types/contextInformation';
...
export class ContextSpy {
  ...
  startCapture(maxCommands = 0, quickCapture = false, fullCapture = false): void {
    ...
    this.currentCapture.canvas.clientWidth =
      (this.context.canvas as HTMLCanvasElement).clientWidth || this.context.canvas.width;
    this.currentCapture.canvas.clientHeight =
      (this.context.canvas as HTMLCanvasElement).clientHeight || this.context.canvas.height;
