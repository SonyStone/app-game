...
export class Texture2DRecorder extends BaseRecorder<WebGLTexture> {
  ...
  protected getBoundInstance(target: number): WebGLTexture {
    ...
    if (target === WebGlConstants.TEXTURE_2D.value) {
      ...
      target === WebGlConstants.TEXTURE_CUBE_MAP_POSITIVE_X.value ||
  ...
  protected delete(instance: WebGLTexture): number {
    ...
    if (
      customData.target === WebGlConstants.TEXTURE_2D_ARRAY.name ||
      customData.target === WebGlConstants.TEXTURE_3D.name
    ) {
      return 0;
    }
  ...
  protected update(functionInformation: IFunctionInformation, target: string, instance: WebGLTexture): number {
    ...
    const previousLength = (instance as any).__SPECTOR_Object_CustomData
      ? (instance as any).__SPECTOR_Object_CustomData.length
      : 0;
    if (customData.isCompressed) {
      ...
      customData.length =
        customData.width * customData.height * cubeMapMultiplier * this.getByteSizeForInternalFormat(internalFormat);
    }
