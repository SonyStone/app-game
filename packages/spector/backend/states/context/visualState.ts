...
export class VisualState extends BaseState {
  static readonly stateName = 'VisualState';

  get stateName(): string {
    return VisualState.stateName;
  }

  static captureBaseSize = 256;
  ...
  protected readFrameBufferAttachmentFromRenderBuffer(
    ...
    if (storage.__SPECTOR_Object_CustomData) {
      ...
      if (
        !samples &&
        !ReadPixelsHelper.isSupportedCombination(componentType, WebGlConstants.RGBA.value, internalFormat)
      ) {
        return;
      }
    } else {
  ...
  protected readFrameBufferAttachmentFromTexture(
    ...
    const textureCubeMapFaceName =
      textureCubeMapFace > 0 ? WebGlConstantsByValue[textureCubeMapFace].name : WebGlConstants.TEXTURE_2D.name;
    ...
    if (textureLayer > 0 || knownAsTextureArray) {
      ...
      gl.framebufferTexture2D(
        WebGlConstants.FRAMEBUFFER.value,
        WebGlConstants.COLOR_ATTACHMENT0.value,
        textureCubeMapFace ? textureCubeMapFace : WebGlConstants.TEXTURE_2D.value,
        storage,
        textureLevel
      );
    }
