 ...
 */
export function prepareComputedTip(size: number, hardness: number, angle: number, roundness: number, softProfile = true) {
  if (![size, hardness, angle, roundness].every(Number.isFinite)) throw new RangeError('Computed tip settings must be finite.');
  const nominal = Math.max(1, Math.min(5000, Math.floor(size + .5)));
  ...
  const enlargement = softProfile ? .013 * Math.trunc(100 - factor * hardness / 100) + 2 : 2;
  const major = nominal === 1 ? 2 : Math.floor((nominal === 2 ? 2.5 : nominal) * enlargement + .5);
  ...
  return { major, minor, cosine: Math.floor(Math.cos(radians) * 16384 + .5),
    sine: Math.floor(Math.sin(radians) * 16384 + .5),
    falloff: Math.trunc((100 - hardness) * (minimum ^ 65535) / 100) + minimum, softProfile };
}
 ...
 */
export function rasterizeComputedTip(tip: ReturnType<typeof prepareComputedTip>, phase: readonly [number, number] = [2, 2],
  context: ComputedTipContext = defaultContext()) {
  const { major, minor, cosine, sine } = tip;
  if (!Number.isInteger(major) || major < 1 || major > 16500 || !Number.isInteger(minor) || minor < 1 || minor > major ||
    !Number.isInteger(cosine) || Math.abs(cosine) > 16384 || !Number.isInteger(sine) || Math.abs(sine) > 16384 ||
    !Number.isInteger(tip.falloff) || tip.falloff < 1 || tip.falloff > 65535 ||
  ...
  const left = (phase[0] - ex + 1) >> 2, top = (phase[1] - ey + 1) >> 2;
  const width = ((phase[0] + ex + 2) >> 2) - left, height = ((phase[1] + ey + 2) >> 2) - top;
  const { bytes, period } = context.rounding;
  if (width * height > 64_000_000 || width < 0 || height < 0 || period <= 0 || period + width > bytes.length || context.profile.length !== 1024)
    throw new RangeError('Computed tip dimensions or rounding context are invalid.');
  const execution = context.execution;
  if (execution && (!Number.isInteger(execution.rowStride) || execution.rowStride < width || execution.rowStride > 0x7fffffff ||
    !Number.isInteger(execution.maximumThreads) || execution.maximumThreads < 1 || execution.maximumThreads > 1024))
    throw new RangeError('Computed tip row stride or execution capacity is invalid.');
  const groupRows = execution && width * height >= 16
    ? Math.max(4, Math.ceil(height / execution.maximumThreads)) : Math.max(1, height);
  const data = new Uint8Array(width * height);
  const ox = 8 - (phase[0] - left * 4) * 4, oy = 8 - (phase[1] - top * 4) * 4;
  let rowX = cosine * ox - sine * oy, rowY = cosine * oy + sine * ox;
  ...
  for (let row = 0; row < height; row++) {
    ...
    for (let column = 0; column < width; column++) {
      const ax = Math.trunc(Math.abs(x | 0) / major), ay = Math.trunc(Math.abs(y | 0) / minor);
      let value = 0;
    ...
    if (rounding + width >= bytes.length) {
      const seed = groupAddress ^ (((Math.imul(groupRow, 0x41c64e6d) + 0x3039) ^ (Math.imul(width, 0x41c64e6d) + 0x3039)) >>> 3);
 ...
 */
export function computedSecondaryTip(size: number, settings: { hardness: number; angle: number; roundness: number }): BrushTipImage & { key: string } {
...
export function computedRadialProfile(): Uint16Array {
  const exponent = Math.log(510), join = (1 + Math.sqrt(1 - 2 / exponent)) * .5;
  const slope = Math.exp(-exponent * join * join) / (1 - join);
  return Uint16Array.from({ length: 1024 }, (_, i) => {
    const radius = i / 1024;
    return Math.floor(65280 * (radius < join ? Math.exp(-exponent * radius * radius) : slope * (1 - radius)) + .5);
  });
}

function hash(value: number) { return (Math.imul(value, 0x41c64e6d) + 0x3039) >>> 3; }
...
const hypotenuse = Uint16Array.from({ length: 257 }, (_, i) => Math.floor(16384 * Math.sqrt(1 + (i / 256) ** 2) + .5));
