/** Measure native pressure override normalization with temporary RGB documents.
 ...
 */
(function () {
    var output = new Folder(Folder.temp + '/paint-native-pressure-overrides-calibration');
    ...
    try {
        ...
        recovery.open('w'); recovery.write(originalOptions.toStream()); recovery.close();
        ...
        var cases = [];
        for (var minimum = 0; minimum <= 80; minimum += 80) {
            for (var jitter = 0; jitter <= 80; jitter += 80) {
                if (minimum === 0 && jitter === 80) continue;
                for (var flags = 0; flags < 4; flags++) {
                    for (var enabled = 0; enabled < 2; enabled++) {
                        cases.push(['min' + minimum + '-jitter' + jitter + '-override' + flags + '-enabled' + enabled,
                            minimum, jitter, flags, !!enabled]);
                    }
                }
            }
        }
        for (var i = 0; i < cases.length; i++) {
            var test = cases[i];
            configure(test[1], test[2], test[3], test[4]);
            log.push(test[0] + '\n' + descriptorJSON(getOptions()));
            doc = app.documents.add(UnitValue(512, 'px'), UnitValue(160, 'px'), 72, 'Brush research ' + test[0], NewDocumentMode.RGB, DocumentFill.WHITE, 1, BitsPerChannelType.EIGHT, 'sRGB IEC61966-2.1');
            var coordinates = [[64,80],[448,80]];
            var points = [];
        ...
        file.encoding = 'UTF8'; file.open('w'); file.write(log.join('\n')); file.close();
    }
    ...
    function setOptions(options) {
        var reference = new ActionReference(); reference.putClass(s('paintbrushTool'));
    ...
    /** Set controlled dynamics before toggling the two native toolbar overrides. */
    function configure(minimum, jitter, flags, enabled) {
        var options = new ActionDescriptor(); options.fromStream(originalOptions.toStream());
        var tip = new ActionDescriptor();
        tip.putUnitDouble(s('diameter'), s('pixelsUnit'), 64);
        ...
        tip.putBoolean(s('flipX'), false); tip.putBoolean(s('flipY'), false);
        options.putObject(s('brush'), s('computedBrush'), tip);
        options.putInteger(s('opacity'), 100); options.putInteger(s('flow'), 100);
        options.putEnumerated(s('mode'), s('blendMode'), s('normal'));
        var disabled = ['useTipDynamics','useScatter','useTexture','useColorDynamics','usePaintDynamics','useBrushPose','usePressureOverridesOpacity','usePressureOverridesSize','wetEdges','noise','repeat','smoothing','pressureSmoothing','brushProjection','flipX','flipY'];
        ...
        options.putBoolean(s('useLegacy'), false);
        var dynamics = new ActionDescriptor();
        dynamics.putInteger(charIDToTypeID('bVTy'), jitter ? 3 : 2);
        dynamics.putInteger(charIDToTypeID('fStp'), 25);
        dynamics.putUnitDouble(s('jitter'), s('percentUnit'), jitter);
        dynamics.putUnitDouble(s('minimum'), s('percentUnit'), minimum);
        options.putObject(charIDToTypeID('szVr'), charIDToTypeID('brVr'), dynamics);
        options.putObject(charIDToTypeID('opVr'), charIDToTypeID('brVr'), dynamics);
        ...
        options.putUnitDouble(s('minimumRoundness'), s('percentUnit'), 25);
        options.putUnitDouble(s('tiltScale'), s('percentUnit'), 200);
        var flowDynamics = new ActionDescriptor();
        flowDynamics.putInteger(charIDToTypeID('bVTy'), 2);
        flowDynamics.putInteger(charIDToTypeID('fStp'), 25);
        flowDynamics.putUnitDouble(s('jitter'), s('percentUnit'), 40);
        flowDynamics.putUnitDouble(s('minimum'), s('percentUnit'), 20);
        options.putObject(charIDToTypeID('prVr'), charIDToTypeID('brVr'), flowDynamics);
        options.putBoolean(s('useTipDynamics'), enabled);
        options.putBoolean(s('usePaintDynamics'), enabled);
        options.putUnitDouble(s('minimumDiameter'), s('percentUnit'), minimum);
        options.putBoolean(s('pressureSmoothing'), false);
        options.putBoolean(s('usePressureOverridesSize'), !!(flags & 1));
        options.putBoolean(s('usePressureOverridesOpacity'), !!(flags & 2));
        setOptions(options);
    }
