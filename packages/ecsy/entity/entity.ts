...
function createComponentsIdStore(): [
  Map<Constructor<Component>, number>,
  (componentConstructor: Constructor<Component>) => void
] {
  let id = 0;
  const components = new Map<Constructor<Component>, number>();

  function add(componentConstructor: Constructor<Component>) {
    if (!components.has(componentConstructor)) {
      components.set(componentConstructor, id++);
    }
  }
...
export const [components, add] = createComponentsIdStore();

// export const components = new ComponentsIdStore();
...
export class Entity implements Component {
  ...
  removeComponent(
    ...
    this.entityManager.entityRemoveComponent(
      this,
  ...
  hasComponent(
    ...
    return (
      ...
      (includeRemoved === true &&
        this.hasRemovedComponent(componentConstructor))
    );
  }
  ...
  hasAllComponents(
    componentConstructors: Map<Constructor<Component>, any>
  ): boolean {
