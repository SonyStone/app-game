...
/** Checks all smoothing modes for light stylus pressure and taper ramps across tile/frame boundaries. */
export async function verifyPressureSpacing(report: (message: string) => void) {
  ...
  try {
    for (const mode of ['studio', 'normal', 'smooth'] as const) {
      for (const size of [32, 128, 512]) {
        for (const ramp of [false, true]) {
          ...
          if (minimum < 125 || maximum > 154)
            throw new Error(
              `${mode} ${size}px ${ramp ? 'ramp' : 'light pressure'} has gaps or excess opacity: ${minimum}–${maximum}`
            );
          if (!ramp && maximum - minimum > 12)
            throw new Error(`${mode} ${size}px light pressure has periodic stamp bands: ${minimum}–${maximum}`);
          report(
            `PASS: ${mode} ${size}px ${ramp ? 'pressure taper' : '4% pressure'} stays continuous across three tile seams; alpha ${minimum}–${maximum}/255`
          );
        }
      }
    }
