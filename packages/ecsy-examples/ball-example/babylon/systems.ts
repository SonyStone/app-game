  ...
  run(entities: [Entity, PulsatingColor, Object3D][], [{ time }]: PerformanceCompensation[]) {
    ...
    for (const [entity, { offset }, { object }] of entities) {
      if (entity.hasComponent(Colliding)) {
        ...
        const r = Math.sin(time / 500 + offset * 12) / 2 + 0.5;
...
@SystemData([Read(Entity), Read(PulsatingScale)], Read(PerformanceCompensation))
...
@SystemData([Read(Object3D), Read(PulsatingScale)], Read(PerformanceCompensation))
