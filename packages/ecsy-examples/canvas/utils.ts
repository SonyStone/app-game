...
export class Intersection implements Resettable {
  ...
  intersect(circleA: Circle, positionA: Position, circleB: Circle, positionB: Position) {
    ...
    a = (circleA.radius * circleA.radius - circleB.radius * circleB.radius + d * d) / (2.0 * d);
...
export function drawLine(ctx: CanvasRenderingContext2D, a: number, b: number, c: number, d: number) {
  ctx.beginPath();
  ctx.moveTo(a, b);
  ctx.lineTo(c, d);
  ctx.stroke();
