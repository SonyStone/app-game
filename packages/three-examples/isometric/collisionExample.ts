...
export function collisionExample(app: Application, world: Container): Container {
  ...
  const redSquare = new Sprite(Texture.WHITE) as SpriteWithPhysics;
  ...
  app.ticker.add((delta) => {
    ...
    const mouseCoords = new Point(
      app.renderer.plugins.interaction.mouse.global.x - world.x,
      app.renderer.plugins.interaction.mouse.global.y - world.y
    );
    // console.log(`mouseCoords`, mouseCoords.x, mouseCoords.y);
  ...
  container.addChild(redSquare, greenSquare);
  return container;
}
...
function testForAABB(object1: Container, object2: Container) {
