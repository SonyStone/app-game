...
/** Executes a validated idle Mixer command without starting a stroke or modifying document history. */
export function runAbrBrushCommand<Layer, Change, Brush extends AbrBrushInput>({ settings, command, brush, renderer, resources, layer, layers }: Omit<AbrStrokeContext<Layer, Change, Brush>, 'processor'> & { command: AbrBrushCommand }) {
...
/** Cloneable preset configuration. Decode before beginning GPU stroke state. */
export const abrBrushSettings = z
  .object({
